...
advanced_resume={0:[]}#0 union {direct_children of parallel nodes} index this. Each one stores a list of children from which relevant resumes can happen (i.e, direct children of sequence nodes excluding the first)
sequence_point={}#each sequence node points to a list in advanced_resume that it references.
sequence_descendant_list = {}
...
def walk_tree(t, parent_id, assigned_id, resume_index = 0, first_child = True, sequence_descendant = False, parallel_unsynch = False):
    global node_count, children, nodes, additional_arguments, needed_nodes, variable_to_int, variable_access, blackboard_needed, last_child, advanced_resume, sequence_point, sequence_descendant_list, parallel_synch_list, parallel_unsych_list, variable_check, external_status_req;
    this_id=assigned_id
    ...
    if (not first_child) and sequence_descendant:
        advanced_resume[resume_index].append(this_id)
        for seq in sequence_descendant:
            sequence_descendant_list[seq].append(this_id)
    ...
    #advanced_resume[resume_node][resume_child].pop()#can only resume from children
    if "node_parallel" == nodes[this_id][0]:
        #counts as a 'leaf' node
        for child in t.children:
            node_count_now = node_count_now + 1
            #each child creates a new resume variable. (some may be empty though)
            if this_id in parallel_synch_list:
                advanced_resume[node_count_now] = [-2]#this will allow us to store the parallel skip information here, without using another variable, decreasing overall state space usage.
            else:
                advanced_resume[node_count_now] = []
            #none of the children are relevant to this resume scheme.
            walk_tree(child, this_id, node_count_now, resume_index, True, -1, parallel_unsynch)
            
    elif "node_sequence" == nodes[this_id][0]:
        if (not first_child) and sequence_descendant:
            advanced_resume[resume_index].pop()#this was not a 'leaf' node
            for seq in sequence_descendant:
                sequence_descendant_list[seq].pop()
        sequence_point[this_id] = resume_index #remember where this node points.
        #first_child = True
        new_seq_list = []
        for seq in sequence_descendant:
            new_seq_list.append(seq)
        new_seq_list.append(this_id)
        sequence_descendant_list[this_id] = []
        for child in t.children:
            node_count_now = node_count_now + 1
            #each child, except the first, needs to be tracked.
            walk_tree(child, this_id, node_count_now, resume_index, first_child, new_seq_list, parallel_unsynch)
            if first_child:
                first_child = False
    elif "node_selector" == nodes[this_id][0]:
        #counts as a 'leaf' node
        for child in t.children:
            node_count_now = node_count_now + 1
            #none of the children are relevant to this resume scheme.
            walk_tree(child, this_id, node_count_now, resume_index, first_child, [], parallel_unsynch)
    elif "decorator" in nodes[this_id][0]:
        if (not first_child) and sequence_descendant:
            advanced_resume[resume_index].pop()#this was not a 'leaf' node
            for seq in sequence_descendant:
                sequence_descendant_list[seq].pop()
        for child in t.children:
            node_count_now = node_count_now + 1
            #if we were a descendant of a sequence, then this will be a descendant of a sequence
            #store first child potential as well
            walk_tree(child, this_id, node_count_now, resume_index, first_child, sequence_descendant, parallel_unsynch)
    else:
        print('unknown node type encountered: ' + nodes[this_id][0])
        print('node name is : ' + nodes[this_id][2])
        #be over conservative. keep this node, and store all information
        for child in t.children:
            node_count_now = node_count_now + 1
            #if we were a descendant of a sequence, then this will be a descendant of a sequence
            #store first child potential as well
...
def main():
    ...
    walk_tree(root, -1, 0, 0, True, [], args.force_parallel_unsynch)
    ...
    parallel_init = ""
    parallel_next = ""
    for index in advanced_resume:
        if len(advanced_resume[index]) == 0:
            #this means we ran into a parallel whose child had no sequence nodes
            pre_nuxmv_string += ("\t\tresume_status_" + str(index) + " := " + str(index) + ";" + os.linesep)
            pass
        elif len(advanced_resume[index]) == 1 and advanced_resume[index][0] == -2:
            #this means a parallel synch node added it
            advanced_resume[index].append(index)#add the child of the parallel_synch node, we will use it as a boolean
            nuxmv_string += "\t\tresume_status_" + str(index) + " : {" + str(advanced_resume[index])[1:-1] + "};" + os.linesep
            parallel_init += "\t\tinit(resume_status_" + str(index) + ") := " + str(index) + ";" + os.linesep
            parallel_next += ("\t\tnext(resume_status_" + str(index) + " ) := " + os.linesep
                              + "\t\t\tcase" + os.linesep
                              + "\t\t\t\t(previous_node = " + str(index) + ") & (previous_status = running | previous_status = failure) : " + str(index) + ";" + os.linesep#if we return failure or running, we don't want to skip this
                              + "\t\t\t\t(previous_node = " + str(index) + ") & (previous_status = success) : -2;" + os.linesep #if we return success, this is skippable
                              + "\t\t\t\t(previous_node = parents[" + str(index) + "]) & (previous_status = failure | previous_status = success) : " + str(index) + ";" + os.linesep#reset if parent is not running
                              + "\t\t\t\t(previous_node = 0) & (previous_status = failure | previous_status = success) : " + str(index) + ";" + os.linesep#reset if overall is not running
                              + "\t\t\t\tTRUE : resume_status_" + str(index) + ";" + os.linesep
                              + "\t\t\tesac;" + os.linesep
            )
            #if we only have 1 item and it's not -2, then it means we have a sequence node with 2 children, because the first was ignored,
        else:
            if index in advanced_resume[index]:
                #this......should not be possible.
                #the only case where index is in advanced_resume[index], is if it's the root case, but then len is 1.
                #oh well. hopefully it'll be obvious
                print("this.....probably shouldn't have happened.")
                print(index)
                print(advanced_resume)
                pass
            else:
                advanced_resume[index].append(index)#add the index back in. this will be used to handle every case where the first child returns running.
            nuxmv_string += "\t\tresume_status_" + str(index) + " : {" + str(advanced_resume[index])[1:-1] + "};" + os.linesep
            parallel_init += "\t\tinit(resume_status_" + str(index) + ") := " + str(index) + ";" + os.linesep
            inject_string = ""
            if -2 in advanced_resume[index]:
                #sync case
                inject_string = ("\t\t\t\t(previous_node = " + str(index) + ") & (previous_status = failure) : " + str(index) + ";" + os.linesep#if we return failure, we don't want to skip this. and also we we can reset. running is handled by the general case
                                 +"\t\t\t\t(previous_node = " + str(index) + ") & (previous_status = success) : -2;" + os.linesep #if we return success, this is skippable
                )
            else:
                inject_string = ("\t\t\t\t(previous_node = " + str(index) + ") & (previous_status = success | previous_status = failure) : " + str(index) + ";" + os.linesep#if we return success or failure, then we reset
                )
                
            parallel_next += ("\t\tnext(resume_status_" + str(index) + " ) := " + os.linesep
                              + "\t\t\tcase" + os.linesep
                              + "\t\t\t\t(resume_status_" + str(index) + "= " +str(index) + ") & (previous_node in {" + str(advanced_resume[index])[1:-1] +"}) & (previous_status = running) : previous_node;" + os.linesep#point to the previous_node if it returned running, is a valid point target, and we aren't pointing to default location (if we don't have the default location check, while returning we will always overwrite the node that actually returned running)
                              + inject_string
                              + "\t\t\t\t (previous_node in {" + str(advanced_resume[index])[1:-1] +"}) & (previous_status = success | previous_status = failure) : " + str(index) + ";" + os.linesep#if we run into a node in this region that has explicitly returned a success or failure, reset (this will allow us to reset if we resume
                              + "\t\t\t\t(previous_node = parents[" + str(index) + "]) & (previous_status = failure | previous_status = success) : " + str(index) + ";" + os.linesep#reset if parent is not running
                              + "\t\t\t\t(previous_node = 0) & (previous_status = failure | previous_status = success) : " + str(index) + ";" + os.linesep#reset if overall is not running
                              + "\t\t\t\tTRUE : resume_status_" + str(index) + ";" + os.linesep
                              + "\t\t\tesac;" + os.linesep
            ...
            #why is this correct?
            #within this region we have no parallel nodes. parallel nodes 'beneath' us are treated as leaf nodes, and parallel nodes 'above' us are treated as -1.
            #normal entry, no running -> obvious
            #normal entry, return running -> we jump to whichever node set it. unless we encounter another leaf node that returns s\f. if the subtree doesn't return running, we reset (-2 in some cases)
            #resume entry, no running -> we jump to the resumed 'leaf'. if it returns s\f, we reset the counter, and continue as normal.
            #resume entry, no running -> we jump to the resumed 'leaf'. if it returns s\f, we reset the counter, and continues as normal (covered by normal entry, return running)
                
    # for seq in sequence_descendant_list:
    #     print(sequence_descendant_list)
    #     if len(sequence_descendant_list[seq]) == 0:
    #         pass
    #     else:
    #         pre_nuxmv_string += ("\t\tsequence_descendant_" + str(seq) + " := {" + str(sequence_descendant_list[seq])[1:-1] + "};" + os.linesep
    #         )
    ...
    for node_id in range(0, node_count):
        ...
        if node_id not in children:
            ...
            #otherwise point to child
        elif node_id in parallel_synch_list:
            for child in children[node_id]:
                post_pre_nuxmv_string += ("\t\t\t\t(previous_node <" + str(child) + ") & !(resume_status_" + str(child) + " = -2) : next_node_" + str(child) + ";" + os.linesep)#if resume_status_ = -2, then we were instructed to skip this child. otherwise, if we're infront of it, point to w/e the child is pointing.
            ...
            for descendant in sequence_descendant_list[node_id]:
                post_pre_nuxmv_string += ("\t\t\t\t(resume_status_" + str(sequence_point[node_id]) + " = " + str(descendant) + ") : next_node_" + str(descendant) + ";" + os.linesep #if the resume points to one of our 'leafs', then jump to wherever it's pointing.
                )
            for child in children[node_id]:
                post_pre_nuxmv_string += ("\t\t\t\t(previous_node <" + str(child) + ") : next_node_" + str(child) + ";" + os.linesep)#otherwise, go through each child in order
