                ...
                (
                    ...
                    + (
                        ...
                        (
                            ...
                            (
                                ('(min, 10000000, (max, -10000000, ' + create_int_value(-1, bl, local, env) + '))')
...
for count in range(TO_GEN):
    ...
    for _ in range(0, random.randint(1, 3)):
        new_var = create_var(total_vars, num_array < 1, 'VAR')
    ...
    for _ in range(0, random.randint(0, 2)):
        new_var = create_var(total_vars, num_array < 0, 'FROZENVAR')
        if new_var['array'] is not None:
            num_array = num_array + 1
        if new_var['type'] == 'bl':
    ...
    for _ in range(0, random.randint(2, 4)):
        new_var = create_var(total_vars, num_array < 2, 'DEFINE')
        if new_var['array'] is not None:
            num_array = num_array + 1
        if new_var['type'] == 'bl':
