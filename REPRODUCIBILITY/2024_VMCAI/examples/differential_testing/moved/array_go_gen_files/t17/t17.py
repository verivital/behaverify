...
def create_blackboard():
    ...
    __temp_var__ = serene_safe_assignment.blVAR0([(0, (
        min(-2, max(-5, (74 - max(-80, -2))))
        if ((False ^ False) == ((not (False)) or (False))) else
        (
        min(-2, max(-5, max(2, -22)))
    ))), (1, (
        min(-2, max(-5, -((([(False == True), (4 != 5)].count(True)) + 4 + -66))))
        if True else
        (
            min(-2, max(-5, abs((([(True and False), (not ((True ^ True))), (False == False)].count(True)) - -(-80)))))
            if (True and False) else
            (
            min(-2, max(-5, -((23 * 75 * -45))))
    )))), (2, min(-2, max(-5, -87)))])
    ...
    def blDEFINE5():
        return min(-2, max(-5, blackboard_reader.blVAR0[2]))
...
def create_tree(environment):
    ...
    sel1 = py_trees.composites.Selector(name = 'sel1', memory = False, children = [a1, c2, a4])
    ...
    p_all2 = py_trees.composites.Parallel(name = 'p_all2', policy = py_trees.common.ParallelPolicy.SuccessOnAll(False), children = [a4_1, a1_1])
    p_all2.tick = parallel_better_tick.__get__(p_all2, py_trees.composites.Parallel)
    c1 = c1_file.c1('c1')
    sel0 = py_trees.composites.Selector(name = 'sel0', memory = True, children = [sel1, p_all2, c1])
