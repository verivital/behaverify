...
def blVAR3(new_value):
    if not isinstance(new_value, bool):
        raise SereneAssignmentException('variable blVAR3 expected type bool but received type ' + str(type(new_value)))
    return new_value
...
def localFROZENVAR5(new_values):
    ...
    for (index, new_value) in new_values:
        if not isinstance(index, int):
            raise SereneAssignmentException('Index must be an int when accessing localFROZENVAR5: ' + str(type(index)))
        if index < 0 or index >= 2:
            raise SereneAssignmentException('Index out of bounds when accessing localFROZENVAR5: ' + str(index))
...
def localFROZENVAR6(new_value):
    if not isinstance(new_value, bool):
        raise SereneAssignmentException('variable localFROZENVAR6 expected type bool but received type ' + str(type(new_value)))
    return new_value
