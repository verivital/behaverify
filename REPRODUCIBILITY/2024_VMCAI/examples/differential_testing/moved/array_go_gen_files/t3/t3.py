...
def create_blackboard():
    ...
    __temp_var__ = serene_safe_assignment.blVAR0([(0, (
        min(-2, max(-5, max(max(-69, 5), 3)))
        if ((not ((72 >= -3))) or (True)) else
        (
            min(-2, max(-5, (37 - (2 - -77))))
            if (False != True) else
            (
    ...
    blackboard_reader.blVAR3 = serene_safe_assignment.blVAR3((
        (blackboard_reader.blVAR0[0] > 7)
        if True else
        (
        (([(97 >= blackboard_reader.blVAR0[0]), (not ((False ^ True))), (False and True)].count(True)) >= (47 + -40))
    )))
