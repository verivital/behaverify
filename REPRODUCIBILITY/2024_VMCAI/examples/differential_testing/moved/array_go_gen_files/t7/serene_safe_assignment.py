...
def envVAR2(new_value):
    if not isinstance(new_value, bool):
        raise SereneAssignmentException('variable envVAR2 expected type bool but received type ' + str(type(new_value)))
    return new_value


def envFROZENVAR3(new_value):
    if not isinstance(new_value, bool):
        raise SereneAssignmentException('variable envFROZENVAR3 expected type bool but received type ' + str(type(new_value)))
    return new_value


def localFROZENVAR4(new_value):
    if not isinstance(new_value, int):
        raise SereneAssignmentException('variable localFROZENVAR4 expected type int but received type ' + str(type(new_value)))
    if new_value >= 2 and new_value <= 5:
        return new_value
    else:
        raise SereneAssignmentException('variable localFROZENVAR4 expected value between 2 and 5 inclusive but received value ' + str(new_value))
