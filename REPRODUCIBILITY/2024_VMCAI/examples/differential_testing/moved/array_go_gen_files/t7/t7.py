...
def create_blackboard():
    ...
    __temp_var__ = serene_safe_assignment.blVAR0([(0, (
        min(-2, max(-5, abs(-5)))
        if (True == ((-5 * 4) > max(-77, 5))) else
        (
            min(-2, max(-5, ((2 * -3 * 5 * -43) - -65)))
            if (True != (not ((False ^ True)))) else
            (
            min(-2, max(-5, abs(([(True and True), (False and True)].count(True)))))
    )))), (1, (
        min(-2, max(-5, abs(-5)))
        if (True == ((-5 * 4) > max(-77, 5))) else
        (
    ...
    blackboard_reader.blDEFINE5 = [None] * 3
    ...
    blackboard_reader.blDEFINE7 = [None] * 2
    ...
    def blDEFINE7(index):
        ...
        if index == 0:
            return (
                (not ((blackboard_reader.blDEFINE5(1) ^ blackboard_reader.blDEFINE5(2))))
                if ((-25 == -(49)) == (False and blackboard_reader.blDEFINE5(1))) else
                (
                    ((not (True)) or ((-78 >= blackboard_reader.blVAR0[0])))
                    if ((50 >= blackboard_reader.blVAR0[1]) == (blackboard_reader.blVAR0[0] < blackboard_reader.blVAR0[1])) else
                    (
                    (blackboard_reader.blDEFINE5(2) == (blackboard_reader.blDEFINE5(1) or blackboard_reader.blDEFINE5(2)))
            )))
        elif index == 1:
            return (
                (blackboard_reader.blVAR0[1] >= abs(27))
                if (True == blackboard_reader.blDEFINE5(0)) else
                (
                    (11 <= blackboard_reader.blVAR0[1])
                    if ((blackboard_reader.blDEFINE5(2) == True) and ((blackboard_reader.blVAR0[2] * 8 * blackboard_reader.blVAR0[2] * 5) > blackboard_reader.blVAR0[2])) else
                    (
                    blackboard_reader.blDEFINE5(1)
            )))
...
def create_tree(environment):
    ...
    sel1 = py_trees.composites.Selector(name = 'sel1', memory = True, children = [a4, a2, a4_1])
    sel1.tick = selector_better_tick.__get__(sel1, py_trees.composites.Selector)
    a1 = a1_file.a1('a1', environment)
    c1 = c1_file.c1('c1')
    sel0 = py_trees.composites.Selector(name = 'sel0', memory = False, children = [sel1, a1, c1])
    sel0.tick = selector_better_tick.__get__(sel0, py_trees.composites.Selector)
    return sel0
