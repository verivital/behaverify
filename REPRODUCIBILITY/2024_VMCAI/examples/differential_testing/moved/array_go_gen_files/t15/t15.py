...
def create_blackboard():
    ...
    blackboard_reader.register_key(key = 'blFROZENVAR3', access = py_trees.common.Access.WRITE)
    blackboard_reader.register_key(key = 'blDEFINE4', access = py_trees.common.Access.WRITE)
    blackboard_reader.blVAR0 = serene_safe_assignment.blVAR0(min(5, max(2, 11)))
    blackboard_reader.blFROZENVAR3 = serene_safe_assignment.blFROZENVAR3((
        min(-2, max(-5, min(max(blackboard_reader.blVAR0, blackboard_reader.blVAR0), max(87, 78))))
        if (not (((not ((True ^ True))) ^ (True == False)))) else
        (
            min(-2, max(-5, ([('both' != 'yes'), ((not ((True or True))) or (False))].count(True))))
            if (((not (False)) or ((-1 >= 5))) ^ ((True == True) ^ ((not (True)) or (True)))) else
            (
            min(-2, max(-5, 4))
    ))))
    blackboard_reader.blDEFINE4 = [None] * 2


    def blDEFINE4(index):
        if not isinstance(index, int):
            raise Exception('Index must be an int when accessing blDEFINE4: ' + str(type(index)))
        if index < 0 or index >= 2:
            raise Exception('Index out of bounds when accessing blDEFINE4: ' + str(index))
        if index == 0:
            return min(-2, max(-5, ([(False == (28 != blackboard_reader.blFROZENVAR3)), (blackboard_reader.blVAR0 < min(min(blackboard_reader.blVAR0, blackboard_reader.blVAR0), blackboard_reader.blVAR0))].count(True))))
        elif index == 1:
            return min(-2, max(-5, ([(False == (28 != blackboard_reader.blFROZENVAR3)), (blackboard_reader.blVAR0 < min(min(blackboard_reader.blVAR0, blackboard_reader.blVAR0), blackboard_reader.blVAR0))].count(True))))
        raise Exception('Reached unreachable state when accessing blDEFINE4: ' + str(index))
...
def create_tree(environment):
    c2 = c2_file.c2('c2')
    c1 = c1_file.c1('c1')
    p_one1 = py_trees.composites.Parallel(name = 'p_one1', policy = py_trees.common.ParallelPolicy.SuccessOnOne(), children = [c2, c1])
    p_one1.tick = parallel_better_tick.__get__(p_one1, py_trees.composites.Parallel)
    dec_sf0 = py_trees.decorators.SuccessIsFailure(name = 'dec_sf0', child = p_one1)
