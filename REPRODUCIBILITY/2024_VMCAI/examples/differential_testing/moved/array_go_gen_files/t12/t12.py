...
def create_blackboard():
    ...
    blackboard_reader.register_key(key = 'blDEFINE3', access = py_trees.common.Access.WRITE)
    ...
    def blDEFINE3():
        return (
            min(5, max(2, (-(-26) * -(-5) * abs(91) * (84 * -2))))
            if True else
            (
            min(5, max(2, -(5)))
        ))

    blackboard_reader.blDEFINE3 = blDEFINE3


    def blDEFINE4():
        return (
            min(5, max(2, (blackboard_reader.blDEFINE3() - blackboard_reader.blDEFINE3())))
            if False else
            (
                min(5, max(2, (-(blackboard_reader.blDEFINE3()) * 42)))
                if ('both' == 'no') else
                (
                min(5, max(2, (blackboard_reader.blDEFINE3() * ([((not ((False and False))) or (False)), ((not ((True == blackboard_reader.blVAR0))) or (blackboard_reader.blVAR0)), (100 > blackboard_reader.blDEFINE3()), ((True ^ blackboard_reader.blVAR0) or (blackboard_reader.blVAR0 ^ blackboard_reader.blVAR0))].count(True)) * ((blackboard_reader.blDEFINE3() - blackboard_reader.blDEFINE3()) - -20) * (blackboard_reader.blDEFINE3() - blackboard_reader.blDEFINE3()))))
        )))
...
def create_tree(environment):
    ...
    p_all0 = py_trees.composites.Parallel(name = 'p_all0', policy = py_trees.common.ParallelPolicy.SuccessOnAll(True), children = [a4, a1])
