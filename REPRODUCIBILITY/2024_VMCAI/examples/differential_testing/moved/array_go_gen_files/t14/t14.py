...
def create_blackboard():
    ...
    def blDEFINE5():
        return (
            min(5, max(2, max(2, 2)))
            if ((not ((blackboard_reader.blVAR0[2] != (blackboard_reader.blVAR0[0] or True)))) or (True)) else
            (
            min(5, max(2, 41))
        ))
...
def create_tree(environment):
    ...
    c2 = c2_file.c2('c2')
    p_one1 = py_trees.composites.Parallel(name = 'p_one1', policy = py_trees.common.ParallelPolicy.SuccessOnOne(), children = [a1, c2])
    p_one1.tick = parallel_better_tick.__get__(p_one1, py_trees.composites.Parallel)
    dec_rf0 = py_trees.decorators.RunningIsFailure(name = 'dec_rf0', child = p_one1)
