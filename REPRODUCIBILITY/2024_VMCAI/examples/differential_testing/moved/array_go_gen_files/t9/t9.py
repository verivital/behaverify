...
def create_blackboard():
    ...
    blackboard_reader.register_key(key = 'blFROZENVAR4', access = py_trees.common.Access.WRITE)
    ...
    blackboard_reader.blFROZENVAR4 = [None] * 2
    __temp_var__ = serene_safe_assignment.blFROZENVAR4([(0, (
        min(5, max(2, (71 + -14 + -4 + 69)))
        if (([(True or blackboard_reader.blVAR3[0]), (True and blackboard_reader.blVAR3[0])].count(True)) <= -63) else
        (
            min(5, max(2, max(-5, -3)))
            if blackboard_reader.blVAR3[1] else
            (
            min(5, max(2, 95))
    )))), (1, (
        min(5, max(2, (71 + -14 + -4 + 69)))
    ...
    for (index, val) in __temp_var__:
        blackboard_reader.blFROZENVAR4[index] = val


    def blDEFINE6():
        return (
            min(-2, max(-5, 72))
            if (abs(blackboard_reader.blFROZENVAR4[1]) >= (6 - -(blackboard_reader.blFROZENVAR4[1]))) else
            (
                min(-2, max(-5, ([((not (False)) or (blackboard_reader.blVAR3[0])), (blackboard_reader.blVAR3[0] and True), (True or blackboard_reader.blVAR3[1])].count(True))))
                if (((True or True) ^ True) and (blackboard_reader.blVAR0 == blackboard_reader.blVAR0)) else
                (
                min(-2, max(-5, -(max(blackboard_reader.blFROZENVAR4[0], -(85)))))
        )))
...
def create_tree(environment):
    ...
    p_all1 = py_trees.composites.Parallel(name = 'p_all1', policy = py_trees.common.ParallelPolicy.SuccessOnAll(False), children = [a1, a4])
    p_all1.tick = parallel_better_tick.__get__(p_all1, py_trees.composites.Parallel)
    p_one0 = py_trees.composites.Parallel(name = 'p_one0', policy = py_trees.common.ParallelPolicy.SuccessOnOne(), children = [c1, p_all1])
