...
bTreeFunctionA1 _ nodeLocation _ _ _ _ oldBlackboard oldEnvironment futureChanges = (returnStatus, [], [], newBlackboard, newEnvironment, newFutureChanges)
  where
    ...
    boardEnvUpdate0 boardEnv
      | not (((- 80) > (71 * ((-5) * ((max (-80) (boardBlVAR0 conditionBlackboard)) * (-32)))))) = boardEnv
      ...
      where
        ...
        privateTempBoardEnv1 = (updateBoardBlVAR2 blackboard False, environment)
          where
            (blackboard, environment) = privateTempBoardEnv0
    returnStatement :: (BTreeBlackboard, BTreeEnvironment) -> BTreeNodeStatus
    returnStatement boardEnv
      | ((-36) < 98) = Failure
      | (sereneXOR (boardBlVAR2 blackboard) (boardBlVAR2 blackboard)) = Running
      | True = Running
      | otherwise = Success
      where
