...
def create_blackboard():
    ...
    blackboard_reader.blVAR0 = serene_safe_assignment.blVAR0((
        min(5, max(2, abs(abs(2))))
        if (False or False) else
        (
            min(5, max(2, 2))
            if True else
            (
            min(5, max(2, min(([(False == (False or True)), ((True and False) == (False and True)), (min(-81, -44) <= min(-5, -64))].count(True)), -99)))
    ))))
    blackboard_reader.blVAR3 = serene_safe_assignment.blVAR3(min(5, max(2, abs(-((3 - -70))))))
...
def create_tree(environment):
    ...
    dec_sf0 = py_trees.decorators.SuccessIsFailure(name = 'dec_sf0', child = a2)
