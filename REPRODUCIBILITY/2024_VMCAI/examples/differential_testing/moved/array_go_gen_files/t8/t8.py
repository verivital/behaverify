...
def create_blackboard():
    ...
    blackboard_reader.register_key(key = 'blFROZENVAR6', access = py_trees.common.Access.WRITE)
    ...
    blackboard_reader.blFROZENVAR6 = [None] * 3
    __temp_var__ = serene_safe_assignment.blFROZENVAR6([(0, (
        'yes'
        if ((22 * 13 * 62) <= (abs(4) - -84)) else
        (
        'yes'
    ))), (1, 'yes'), (2, (
        'both'
        if (('both' != 'both') != ((not ((True and False))) or ((False != False)))) else
        (
            'yes'
            if (((5 + 66 + 3 + -5) <= (-5 + 5 + 18 + 32)) and ((not ((blackboard_reader.blVAR0 ^ True))) == (-5 <= -22))) else
            (
            'yes'
    ))))])
    for (index, val) in __temp_var__:
        blackboard_reader.blFROZENVAR6[index] = val


    def blDEFINE8():
        return blackboard_reader.blFROZENVAR6[0]
