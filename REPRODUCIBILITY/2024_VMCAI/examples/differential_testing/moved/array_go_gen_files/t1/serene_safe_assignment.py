...
def envVAR2(new_values):
    def envVAR2__internal_type_check(new_value):
        if not isinstance(new_value, int):
            raise SereneAssignmentException('variable envVAR2 expected type int but received type ' + str(type(new_value)))
        if new_value >= -5 and new_value <= -2:
            return new_value
        else:
            raise SereneAssignmentException('variable envVAR2 expected value between -5 and -2 inclusive but received value ' + str(new_value))
...
def envVAR3(new_value):
    ...
    if new_value >= 2 and new_value <= 5:
        ...
        raise SereneAssignmentException('variable envVAR3 expected value between 2 and 5 inclusive but received value ' + str(new_value))


def envFROZENVAR4(new_values):
    def envFROZENVAR4__internal_type_check(new_value):
        if not isinstance(new_value, int):
            raise SereneAssignmentException('variable envFROZENVAR4 expected type int but received type ' + str(type(new_value)))
        if new_value >= 2 and new_value <= 5:
            return new_value
        else:
            raise SereneAssignmentException('variable envFROZENVAR4 expected value between 2 and 5 inclusive but received value ' + str(new_value))

    return_pairs = []
    seen_indices = set()
    for (index, new_value) in new_values:
        if not isinstance(index, int):
            raise SereneAssignmentException('Index must be an int when accessing envFROZENVAR4: ' + str(type(index)))
        if index < 0 or index >= 2:
            raise SereneAssignmentException('Index out of bounds when accessing envFROZENVAR4: ' + str(index))
        checked_value = envFROZENVAR4__internal_type_check(new_value)
        if index not in seen_indices:
            seen_indices.add(index)
            return_pairs.append((index, checked_value))
    return return_pairs
