...
def envFROZENVAR3(new_values):
    def envFROZENVAR3__internal_type_check(new_value):
        if not isinstance(new_value, str):
            raise SereneAssignmentException('variable envFROZENVAR3 expected type str but received type ' + str(type(new_value)))
        if new_value in ['yes', 'no', 'both']:
            return new_value
        else:
            raise SereneAssignmentException("variable envFROZENVAR3 expected value in ['yes', 'no', 'both'] but received value '" + new_value + "'")
...
def envFROZENVAR4(new_values):
    def envFROZENVAR4__internal_type_check(new_value):
        ...
        if new_value >= -5 and new_value <= -2:
            ...
            raise SereneAssignmentException('variable envFROZENVAR4 expected value between -5 and -2 inclusive but received value ' + str(new_value))
