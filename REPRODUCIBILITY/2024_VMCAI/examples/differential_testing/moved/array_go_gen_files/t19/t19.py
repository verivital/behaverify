...
def create_blackboard():
    ...
    blackboard_reader.blVAR0 = serene_safe_assignment.blVAR0(min(5, max(2, 4)))
    blackboard_reader.blDEFINE6 = [None] * 2
...
def create_tree(environment):
    ...
    p_one2 = py_trees.composites.Parallel(name = 'p_one2', policy = py_trees.common.ParallelPolicy.SuccessOnOne(), children = [c2, c1_1, a1])
    p_one2.tick = parallel_better_tick.__get__(p_one2, py_trees.composites.Parallel)
    a1_1 = a1_file.a1('a1_1', environment)
    c2_1 = c2_file.c2('c2_1')
    p_all3 = py_trees.composites.Parallel(name = 'p_all3', policy = py_trees.common.ParallelPolicy.SuccessOnAll(True), children = [a1_1, c2_1])
    p_all3.tick = parallel_better_tick.__get__(p_all3, py_trees.composites.Parallel)
    seq1 = py_trees.composites.Sequence(name = 'seq1', memory = True, children = [c1, p_one2, p_all3])
    seq1.tick = sequence_better_tick.__get__(seq1, py_trees.composites.Sequence)
    c2_2 = c2_file.c2('c2_2')
    dec_fr4 = py_trees.decorators.FailureIsRunning(name = 'dec_fr4', child = c2_2)
    dec_fr4.tick = decorator_better_tick.__get__(dec_fr4, py_trees.decorators.Decorator)
    p_one0 = py_trees.composites.Parallel(name = 'p_one0', policy = py_trees.common.ParallelPolicy.SuccessOnOne(), children = [seq1, dec_fr4])
