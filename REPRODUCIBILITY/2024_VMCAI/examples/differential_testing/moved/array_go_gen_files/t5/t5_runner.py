...
import py_trees
import t5
import t5_environment

blackboard_reader = t5.create_blackboard()
environment = t5_environment.t5_environment(blackboard_reader)
root = t5.create_tree(environment)
...
def print_blackboard():
    ...
    ret_string += indent(1) + 'blFROZENVAR4: ' + str(blackboard_reader.blFROZENVAR4) + os.linesep
    ret_string += indent(1) + 'blDEFINE6: ' + str(blackboard_reader.blDEFINE6()) + os.linesep
    return ret_string
...
node_to_locals = {
    'a1' : [{'name' : 'localVAR2', 'is_func' : False, 'array_size' : None}],
    'a2' : [{'name' : 'localVAR2', 'is_func' : False, 'array_size' : None}],
    'a3' : [],
}
