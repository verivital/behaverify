...
def create_blackboard():
    ...
    blackboard_reader.register_key(key = 'blVAR4', access = py_trees.common.Access.WRITE)
    ...
    __temp_var__ = serene_safe_assignment.blVAR0([(0, (
        ...
        (
        (5 <= abs(-27))
    ))), (1, (4 < ([(False or False), (not ((True ^ False))), (4 > -79), (86 > 5)].count(True)))), (2, (-7 <= -50))])
    for (index, val) in __temp_var__:
        blackboard_reader.blVAR0[index] = val
    blackboard_reader.blVAR3 = serene_safe_assignment.blVAR3(min(5, max(2, 5)))
    blackboard_reader.blVAR4 = serene_safe_assignment.blVAR4((
        min(5, max(2, blackboard_reader.blVAR3))
        if ((blackboard_reader.blVAR3 + blackboard_reader.blVAR3 + 42 + 22) <= min(74, blackboard_reader.blVAR3)) else
        (
        min(5, max(2, 48))
    )))
    ...
    def blDEFINE5():
        return (
            min(5, max(2, min(blackboard_reader.blVAR4, blackboard_reader.blVAR4)))
            if (blackboard_reader.blVAR4 <= ([(not ((blackboard_reader.blVAR0[1] ^ blackboard_reader.blVAR0[2]))), (blackboard_reader.blVAR4 > 52)].count(True))) else
            (
                min(5, max(2, abs(blackboard_reader.blVAR4)))
                if ((True != blackboard_reader.blVAR0[2]) != True) else
                (
                min(5, max(2, max(-19, blackboard_reader.blVAR4)))
        )))

    blackboard_reader.blDEFINE5 = blDEFINE5
    blackboard_reader.blDEFINE6 = [None] * 3


    def blDEFINE6(index):
        if not isinstance(index, int):
            raise Exception('Index must be an int when accessing blDEFINE6: ' + str(type(index)))
        if index < 0 or index >= 3:
            raise Exception('Index out of bounds when accessing blDEFINE6: ' + str(index))
        if index == 0:
            return (-34 > blackboard_reader.blVAR3)
        elif index == 1:
            return (
                (True == (blackboard_reader.blVAR0[2] ^ False))
                if (False != False) else
                (
                    ((([(False and blackboard_reader.blVAR0[0]), ((not (True)) or (True))].count(True)) < 97) or (43 <= blackboard_reader.blVAR4))
                    if (blackboard_reader.blVAR0[1] and False) else
                    (
                    True
            )))
        elif index == 2:
            return (
                blackboard_reader.blVAR0[0]
                if True else
                (
                    (blackboard_reader.blVAR0[0] and (blackboard_reader.blDEFINE5() >= (-94 + blackboard_reader.blVAR3 + blackboard_reader.blDEFINE5() + 83)))
                    if (40 < blackboard_reader.blVAR3) else
                    (
                    blackboard_reader.blVAR0[2]
            )))
        raise Exception('Reached unreachable state when accessing blDEFINE6: ' + str(index))
...
def create_tree(environment):
    ...
    p_one2 = py_trees.composites.Parallel(name = 'p_one2', policy = py_trees.common.ParallelPolicy.SuccessOnOne(), children = [a4, a1])
    p_one2.tick = parallel_better_tick.__get__(p_one2, py_trees.composites.Parallel)
    p_all1 = py_trees.composites.Parallel(name = 'p_all1', policy = py_trees.common.ParallelPolicy.SuccessOnAll(False), children = [a2, c2, p_one2])
    p_all1.tick = parallel_better_tick.__get__(p_all1, py_trees.composites.Parallel)
    dec_sf0 = py_trees.decorators.SuccessIsFailure(name = 'dec_sf0', child = p_all1)
