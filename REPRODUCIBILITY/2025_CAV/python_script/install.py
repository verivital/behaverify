...
def create_image_and_container(dockerfile_path, nuxmv_url, local):
    ...
    if local:
        ...
        if not copy_into(behaverify, nuxmv_url, TEST_DIR + '/'):
            raise RuntimeError('Failed to copy nuXmv into the container.')
        print('End: Adding nuXmv to container: ' + CONTAINER_NAME)
    else:
        serene_exec(behaverify, 'wget ' + nuxmv_url + ' -O ' + TEST_DIR + '/nuXmv_DL.tar.xz', 'Downloading nuXmv from provided URL.', True)
        serene_exec(
            behaverify,
...
if __name__ == '__main__':
    ...
    create_image_and_container(args.input_path, args.nuxmv_url, args.local)
