    ...
    status_module = (
        ...
        + (
            (
                ...
                + indent(4) + 'TRUE : {' + statuses.replace('_', ', ') + '};' + os.linesep
                + indent(3) + 'esac;' + os.linesep
            )
            ...
            (
                ...
                + (
                    ...
                    if statuses == '' else
                    (indent(2) + 'status := active ? ' + statuses + ' : invalid;' + os.linesep)
                )
            )
        )
    )
...
def create_decorator_X_is_Y(ignored_value = 0):
    ...
    return_string = ('MODULE decorator_X_is_Y(child_0, incoming_status, outgoing_status)' + os.linesep
                     + status_start
                     + indent(4) + 'child_0.status = incoming_status : outgoing_status;' + os.linesep  # if we detect the incoming status, use outgoing_status
...
def create_decorator_inverter(ignored_value = 0):
    ...
    return_string = ('MODULE decorator_inverter(child_0)' + os.linesep
                     + status_start
                     + indent(4) + 'child_0.status = success : failure;' + os.linesep  # if we detect the incoming status, use outgoing_status
                     + indent(4) + 'child_0.status = failure : success;' + os.linesep  # if we detect the incoming status, use outgoing_status
                     + indent(4) + 'TRUE : child_0.status;' + os.linesep  # otherwise matcch the child
                     + status_end
                     + active[0]  # handles (if this node not active, child_0 not active)
                     + active_end  # handle (otherwise, child_0 active)
...
def create_composite_selector_with_partial_memory(number_of_children):
    ...
    for child in range(number_of_children):
        ...
        return_string += (indent(4) + '(' + str(child) + ' >= resume_point) & !(child_' + str(child) + '.status = failure) : child_' + str(child) + '.status;' + os.linesep
...
def create_composite_selector_without_memory(number_of_children):
    ...
    for child in range(number_of_children):
        ...
        return_string += (indent(4) + '!(child_' + str(child) + '.status = failure) : child_' + str(child) + '.status;' + os.linesep
...
def create_composite_sequence_with_partial_memory(number_of_children):
    ...
    for child in range(number_of_children):
        ...
        # if none of these occur, it is an error.
        return_string += (indent(4) + '(' + str(child) + ' >= resume_point) & !(child_' + str(child) + '.status = success) : child_' + str(child) + '.status;' + os.linesep
...
def create_composite_sequence_without_memory(number_of_children):
    ...
    for child in range(number_of_children):
        ...
        return_string += (indent(4) + '!(child_' + str(child) + '.status = success) : child_' + str(child) + '.status;' + os.linesep
...
def create_composite_parallel_success_on_all_with_partial_memory(number_of_children):
    ...
    for child in range(number_of_children):
        return_string += indent(4) + '(child_' + str(child) + '.status = failure) & !(skip_child[' + str(child) + '] = -2) : failure;' + os.linesep
    ...
    for child in range(number_of_children):
        ...
        return_string += (indent(4) + '!(child_' + str(child) + '.status = success) & !(skip_child[' + str(child) + '] = -2) : child_' + str(child) + '.status;' + os.linesep
...
def create_composite_parallel_success_on_all_without_memory(number_of_children):
    ...
    for child in range(number_of_children):
        return_string += indent(4) + '(child_' + str(child) + '.status = failure) : failure;' + os.linesep
        # we found failure, therefore, return failure
...
def create_composite_parallel_success_on_one_without_memory(number_of_children):
    ...
    for child in range(number_of_children):
        # we return on first success if success_on_all is false, on first running if it's true, and otherwise we need to see them all
        return_string += (indent(4) + '!(child_' + str(child) + '.status = running) : child_' + str(child) + '.status;' + os.linesep
                          # we found something other than running, so return that.
                          )
    return_string += (indent(4) + 'TRUE : running;' + os.linesep
                      + status_end
