...
for encoding_code in encoding_codes:
    ...
    for i in range(max(len(folder_names), len(file_names), len(mins), len(maxs), len(steps), len(encoding_codes))):
        ...
        df.to_latex('../examples/' + group_name + '/processed_data/tables/' + encoding_code + '/' + file_name + '_diameters.tex', caption=file_name + ', System Diameter', label=file_name + '_diam')

        df = pd.DataFrame(reachable_states, columns=experiments, index=encodings)
        df.to_latex('../examples/' + group_name + '/processed_data/tables/' + encoding_code + '/' + file_name + '_reachable_states.tex', caption=file_name + ', Reachable States', label=file_name + '_reach')

        df = pd.DataFrame(total_v1_states, columns=experiments, index=encodings)
        df.to_latex('../examples/' + group_name + '/processed_data/tables/' + encoding_code + '/' + file_name + '_total_states.tex', caption=file_name + ', Total States', label=file_name + '_total')

        df = pd.DataFrame(elapsed_time_print_states, columns=experiments, index=encodings)
        df.to_latex('../examples/' + group_name + '/processed_data/tables/' + encoding_code + '/' + file_name + '_elapsed_print_states.tex', caption=file_name + ', Time in Seconds to Print Reachability', label=file_name + '_states_time')

        df = pd.DataFrame(elapsed_time_compute_reachable, columns=experiments, index=encodings)
        df.to_latex('../examples/' + group_name + '/processed_data/tables/' + encoding_code + '/' + file_name + '_elapsed_reachable.tex', caption=file_name + ', Time in Seconds to Compute Reachability', label=file_name + '_states_time')

        df = pd.DataFrame(elapsed_time_ctl, columns=experiments, index=encodings)
        df.to_latex('../examples/' + group_name + '/processed_data/tables/' + encoding_code + '/' + file_name + '_elapsed_ctl.tex', caption=file_name + ', Time in Seconds to Compute CTL', label=file_name + '_CTL_time')
        ...
        for thing in ['diameters', 'reachable', 'total', 'print', 'compute', 'ctl', 'ltl', 'model', 'ctl_r', 'ltl_r']:
            ...
            plt.tight_layout()
            plt.savefig('../examples/' + group_name + '/processed_data/pictures/' + encoding_code + '/' + file_name + '_' + file_end + '.png')
