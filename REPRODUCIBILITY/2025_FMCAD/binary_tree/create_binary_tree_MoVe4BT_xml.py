...
def create_tree(levels):
    ...
    return (
        ...
        + '<root _reach_goal="" env="True" expected_execution_time="" global_vars="var vv=0;" main_tree_to_execute="BehaviorTree" specification="#assert BehaviorTree |= G(AtMax_s -&gt; G(!(AtMax_f)));">' + os.linesep
        + indent(1) + '<!-- ////////// -->' + os.linesep
        + indent(1) + '<BehaviorTree ID="BehaviorTree">' + os.linesep
        + indent(2) + '<Sequence>' + os.linesep
        + tree
        + indent(3) + '<Fallback>' + os.linesep
        + indent(4) + '<Condition ID="AtMax" failure_guard="vv!=' + str(max_val) + '" success_guard="vv==' + str(max_val) + '"/>' + os.linesep
        ...
        + indent(2) + '<Condition ID="AtMax">' + os.linesep
        + indent(3) + '<inout_port name="failure_guard"/>' + os.linesep
        + indent(3) + '<inout_port name="success_guard"/>' + os.linesep
        + indent(2) + '</Condition>' + os.linesep
        + post
