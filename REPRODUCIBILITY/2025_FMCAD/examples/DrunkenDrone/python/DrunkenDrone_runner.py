...
import py_trees
import DrunkenDrone
import DrunkenDrone_environment
...
def print_blackboard(blackboard_reader):
    ...
    ret_string += indent(1) + 'act: ' + str(blackboard_reader.act) + os.linesep
    ret_string += indent(1) + 'new: ' + str(blackboard_reader.new) + os.linesep
    return ret_string


def print_environment(environment):
    ret_string = 'environment' + os.linesep
    ret_string += indent(1) + 'x_d: ' + str(environment.x_d) + os.linesep
    ret_string += indent(1) + 'y_d: ' + str(environment.y_d) + os.linesep
    ret_string += indent(1) + 'x_g: ' + str(environment.x_g) + os.linesep
    ret_string += indent(1) + 'y_g: ' + str(environment.y_g) + os.linesep
    return ret_string
...
def run_tree():
    ...
    blackboard_reader = DrunkenDrone.create_blackboard(serene_randomizer)
    environment = DrunkenDrone_environment.DrunkenDrone_environment(blackboard_reader)
    serene_randomizer.set_blackboard_and_environment(blackboard_reader, environment)
    DrunkenDrone.initialize_blackboard(blackboard_reader)
    environment.initialize_environment()
    root = DrunkenDrone.create_tree(environment)
