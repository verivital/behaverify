...
def create_runner(blackboard_variables, environment_variables, max_iter, no_var_print, py_tree_print):
    global PROJECT_NAME, PROJECT_ENVIRONMENT_NAME, NO_SERENE_PRINT
    return (
...
def main():
    ...
    global NO_SERENE_PRINT
    NO_SERENE_PRINT = args.no_serene_print
    ...
    with open(args.location + PROJECT_NAME + '_runner.py', 'w') as f:
        f.write(create_runner(list(filter(lambda x : is_blackboard(x), model.variables)), list(filter(lambda x : is_env(x), model.variables)), args.max_iter, args.no_var_print, args.py_tree_print))
    return
