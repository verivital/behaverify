...
def handle_smv(the_file):
    smv_run = []
    smv_arrays_to_build = {}
    smv_locations_to_name = {}
    initial_state = {}
    initial_arrays = {}
            ...
            if 'State: ' in line:
                ...
                if len(smv_run) > 0:
                    for array_name in smv_arrays_to_build:
                        cur_array = []
                        for index in range(len(smv_arrays_to_build[array_name])):
                            cur_array.append(smv_arrays_to_build[array_name][index])
                        smv_run[-1][array_name] = '[' + ', '.join(cur_array) + ']'
                smv_run.append({})
                smv_arrays_to_build = {}
                ignore = False
            elif not ignore:
                ...
                if '.status' in line:
                    ...
                    if index >= 0:
                        if var_stage == 0 and len(smv_run) == 1:
                        ...
                        if var_name in max_stage:
                            if var_stage >= max_stage[var_name]:
                                ...
                                if var_name in smv_arrays_to_build:
                                    smv_arrays_to_build[var_name][index] = var_val
                                else:
                                    smv_arrays_to_build[var_name] = {index : var_val}
                            else:
                                pass
                    ...
                    smv_locations_to_name[node_location] = node_name
    return ([initial_state] + smv_run, smv_locations_to_name, max_stage)

def handle_haskell(the_file):
    the_run = []
    with open(the_file, 'r', encoding = 'utf-8') as f:
        ...
        for line in f:
            ...
            #     the_run[-1][var_name] = '[' + ', '.join(map(lambda index: arrays_to_build[var_name][index], range(len(arrays_to_build[var_name])))) + ']'
    return the_run


def handle_python(the_file):
    the_run = []
    the_nodes = set()
    the_variables = set()
    with open(the_file, 'r', encoding = 'utf-8') as f:
        for line in f:
            ...
            if 'State after tick:' in line or 'Initial State' in line:
                ...
                node_name = node_name.strip()
                the_run[-1][node_name] = status
                the_nodes.add(node_name)
            elif ':' in line:
                ...
                var_val = var_val.strip().upper().replace('\'', '').replace('"', '')
                the_run[-1][var_name] = var_val
                the_variables.add(var_name)
    return (the_run, the_nodes, the_variables)
...
if python_file is not None:
    (python_run, nodes, variables) = handle_python(python_file)
else:
    python_run = []
    nodes = set()
    variables = set()
...
if opt_smv_file is not None:
    (opt_nuxmv_run, _, opt_nuxmv_max_stage) = handle_smv(opt_smv_file)
else:
    opt_nuxmv_run = []
    opt_nuxmv_max_stage = 0
haskell_run = [] if haskell_file is None else handle_haskell(haskell_file)
