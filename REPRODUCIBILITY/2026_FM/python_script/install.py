import argparse
import docker
from docker_util import IMAGE_NAME, CONTAINER_NAME, TEST_DIR, serene_exec, copy_into, HOME_DIR
...
def create_image_and_container(dockerfile_path, nuxmv_loc, local):
    ...
    if nuxmv_loc is not None:
        if local:
            ...
            serene_exec(
                behaverify,
                'tar -xf ' + HOME_DIR + '/nuXmv_DL.tar.xz' + ' --one-top-level=nuXmv_DL --strip-components 1',
                'Extracting nuXmv from archive.',
                True
            )
            serene_exec(behaverify, 'mv ' + HOME_DIR + '/nuXmv_DL/bin/nuXmv' + ' ' + TEST_DIR + '/nuXmv', 'Moving nuXmv from extracted location.', True)
