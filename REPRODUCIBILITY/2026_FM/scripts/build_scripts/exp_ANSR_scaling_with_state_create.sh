...
./make_folder_structure.sh ANSR_scaling_with_state
...
for (( num=min_val; num<=max_val; num=(num + step_size) )); do
    ...
    ./make_optimized_smv.sh "${path_name}" "ANSR_scaling_with_state_flat_${num}"
    ./make_optimized_smv.sh "${path_name}" "ANSR_scaling_with_state_square_${num}"
done
