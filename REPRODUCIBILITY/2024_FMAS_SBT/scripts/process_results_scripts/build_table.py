...
reach = re.compile(r'reachable states: (?P<val1>((\d+)|(\d+(\.\d+)?e\+\d+))) \(2\^((\d+)|(\d+\.\d+))\) out of (?P<val3>((\d+)|(\d+(\.\d+)?e\+\d+))) \(2\^((\d+)|(\d+\.\d+))\)')
elapsed = re.compile(r'elapse: -?(?P<val1>[\.\d]+) seconds,')
resident = re.compile(r'Maximum resident size\s*= (?P<val1>[\.\d]+)K')
