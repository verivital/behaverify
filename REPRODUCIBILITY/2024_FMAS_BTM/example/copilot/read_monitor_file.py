...
class read_monitor(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        temp = collision_monitor_lib.collision_interface(self.blackboard.drone_x, self.blackboard.drone_y, self.blackboard.delta_x(), self.blackboard.delta_y(), self.blackboard.drone_speed)
        ...
        temp = loop_monitor_lib.loop_interface(ACTION_TO_INT[self.blackboard.current_action])
        self.blackboard.monitor_var = ('unsafe' if temp == 1 else 'unknown')
        self.blackboard.drone_speed = (
            1
            if (self.blackboard.monitor_var == 'unsafe') else
