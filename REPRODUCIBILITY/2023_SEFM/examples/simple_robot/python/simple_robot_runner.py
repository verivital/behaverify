...
def draw_board(my_count, board, env):
    print('_________________________________________')
    print('iteration: ' + str(my_count))
    to_print = ''
    for row in range(0, 10):
        for col in range(0, 10):
            if (row == env.y_true) and (col == env.x_true):
                to_print += 'R'
            elif (row == env.y_goal) and (col == env.x_goal) and (env.remaining_goals > 0):
                to_print += 'F'
            else:
                to_print += '-'
        to_print += os.linesep
    print(to_print)
    print('goals left: ' + str(env.remaining_goals))
    print('_________________________________________')
    return
...
for count in range(100):
    ...
    if environment.check_tick_condition():
        full_tick()
        draw_board(count, blackboard_reader, environment)
