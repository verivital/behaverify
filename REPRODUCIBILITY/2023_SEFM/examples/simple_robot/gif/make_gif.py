...
# Read the text file
file_path = "./run.txt"  # Update with the path to your text file
...
start_location = 0
count = 0
lines_per_iter = 15 # use 5 for complex robot
while start_location < len(text):
    end_location = min(len(text), start_location + lines_per_iter)
    text_to_image(text[start_location : end_location], './images/' + str(count) + '.png', 16, 5, (0, 0, 0), (255, 255, 255), font_path)
    start_location = start_location + lines_per_iter
    count = count + 1
...
frame_one = frames[0]
frame_one.save('./robot.gif', format = 'GIF', append_images = frames, save_all = True, duration = 450, loop = 0)
