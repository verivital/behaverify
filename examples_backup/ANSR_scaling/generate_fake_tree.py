...
def insert_net(net):
    ...
    return_string = return_string.replace('} end_specifications', 'INVARSPEC { (eq, 0, fake_net_output)}' + os.linesep + '} end_specifications')
    return return_string

def write_flat_file(hidden_size):
    with open(WRITE_LOCATION + '/ANSR_scaling_flat_' + str(hidden_size) + '.tree', 'w', encoding = 'utf-8') as output:
        output.write(insert_net(create_flat_fake_net(hidden_size * hidden_size)))
    return

def write_square_file(hidden_size):
    with open(WRITE_LOCATION + '/ANSR_scaling_square_' + str(hidden_size) + '.tree', 'w', encoding = 'utf-8') as output:
