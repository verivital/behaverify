...
bTreeFunctionRth2bb :: [BTreeNode] -> TreeLocation -> TrueMemoryStatus -> [TrueMemoryStorage] -> PartialMemoryStatus -> [PartialMemoryStorage] -> BTreeBlackboard -> BTreeEnvironment -> FutureChanges -> BTreeNodeOutput
bTreeFunctionRth2bb _ nodeLocation _ _ _ _ oldBlackboard oldEnvironment futureChanges = (returnStatus, [], [], newBlackboard, newEnvironment, newFutureChanges)
  where
    ...
    boardEnvUpdate0 boardEnv
      ...
      where
        ...
        privateTempBoardEnv2 = (updateBoardBbRthWarning blackboard ((boardBbRthWarning blackboard) || (envBbRth environment)), environment)
          where
