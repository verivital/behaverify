...
bTreeFunctionFls2bb :: [BTreeNode] -> TreeLocation -> TrueMemoryStatus -> [TrueMemoryStorage] -> PartialMemoryStatus -> [PartialMemoryStorage] -> BTreeBlackboard -> BTreeEnvironment -> FutureChanges -> BTreeNodeOutput
bTreeFunctionFls2bb _ nodeLocation _ _ _ _ oldBlackboard oldEnvironment futureChanges = (returnStatus, [], [], newBlackboard, newEnvironment, newFutureChanges)
  where
    ...
    boardEnvUpdate0 boardEnv
      ...
      where
        ...
        privateTempBoardEnv2 = (updateBoardObstacleStandoffWarning blackboard ((envFlsRange environment) == "danger_zone"), environment)
          where
