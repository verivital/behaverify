  ...
  where
    ...
    bTreeFunctionMove _ nodeLocation _ _ _ _ oldBlackboard oldEnvironment futureChanges = (returnStatus, [], [], newBlackboard, newEnvironment, newFutureChanges)
      where
        ...
        boardEnvUpdate0 boardEnv = (updateBoardCurX blackboard (max 0 (min 10 (delta_x + (boardCurX blackboard)))), environment)
          where
            (blackboard, environment) = boardEnv
        boardEnvUpdate1 :: (BTreeBlackboard, BTreeEnvironment) -> (BTreeBlackboard, BTreeEnvironment)
        boardEnvUpdate1 boardEnv = (updateBoardCurY blackboard (max 0 (min 10 (delta_y + (boardCurY blackboard)))), environment)
          where
            (blackboard, environment) = boardEnv
