...
class light_controller_environment():
    ...
    def post_tick_environment_update(self):
        self.tunnel_state = serene_safe_assignment.tunnel_state((
            random.choice(['empty', 'west_to_east'])
            if self.west_light else
            (
                random.choice(['empty', 'east_to_west'])
                if self.east_light else
                (
                random.choice(['empty', self.tunnel_state])
        ))))
    ...
    def __init__(self, blackboard):
        ...
        self.west_light = serene_safe_assignment.west_light(False)
        self.east_light = serene_safe_assignment.east_light(False)
    ...
    def check_safety(self, node):
        ...
        This method is expected to return True or False.
        This method is being modeled using the following behavior:
        ((self.tunnel_state == 'empty') or (self.blackboard.direction == self.tunnel_state))
        -- SIDE EFFECTS
        This method is expected to have no side effects (for the tree).
        '''
        # below we include an auto generated attempt at implmenting this
        return ((self.tunnel_state == 'empty') or (self.blackboard.direction == self.tunnel_state))

    def light_signal_func__0(self, node):
        self.west_light = serene_safe_assignment.west_light((
            True
    ...
    def light_signal_func__1(self, node):
        self.east_light = serene_safe_assignment.east_light((
            True
            if (self.blackboard.direction == 'east_to_west') else
            (
            False
        )))
        return
