...
root = array_test.create_tree(environment)
...
def print_blackboard():
    ...
    ret_string += indent(1) + 'foo: ' + str(blackboard_reader.foo) + os.linesep
    ret_string += indent(1) + 'trom: ' + str([blackboard_reader.trom(x) for x in range(2)]) + os.linesep
    ret_string += indent(1) + 'index_var: ' + str(blackboard_reader.index_var) + os.linesep
    return ret_string
...
node_to_locals = {
    'move_index' : [],
    'action_one' : [{'name' : 'bar', 'is_func' : False, 'array_size' : 3}],
    'action_two' : [{'name' : 'bar', 'is_func' : False, 'array_size' : 3}],
}
