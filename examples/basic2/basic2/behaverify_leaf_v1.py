...
#current plan: replace random_status with an IVAR random_status. should be very easy to change tbh
...
import node_creator_leaf_v1 as node_creator
...
def create_nodes(nodes, children, additional_arguments, parallel_synch_set, parallel_unsynch_set, sequence_set, selector_set, decorator_set, leaf_set):
    ...
    for parallel_node in parallel_synch_set:
        ...
        if parallel_node in children and len(children[parallel_node]) >= 0:
            ...
            #we need to know what the actual last child is, and it changes for parallel nodes because of skipping stuff.
            define_string += ("\t\tlast_child_" + str(parallel_node) + " := " + os.linesep
                              + "\t\t\tcase" + os.linesep
            )
            for child_index in range(len(children[parallel_node])-1, -1, -1):
                define_string += "\t\t\t\t(resume_from_node_" + str(children[parallel_node][child_index]) + " = " + str(children[parallel_node][child_index]) + ") : " + str(children[parallel_node][child_index]) + ";" + os.linesep
...
def create_resume_structure(nodes, local_root_to_relevant_list_map):
    ...
    for local_root in local_root_to_relevant_list_map:
        ...
        next_resume_from_node_string += "\t\t\t\t(statuses[max(0, resume_from_node_" + str(local_root) + ")] in {success, failure}) : " + str(local_root) + ";" + os.linesep#if the node we were planning to resume from has returned success or failure, then reset
