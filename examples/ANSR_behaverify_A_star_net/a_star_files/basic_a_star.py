...
def a_star(grid, min_val, max_val, start_location, end_location):
    ...
    while len(frontier) > 0 and ((grid_cost[e_x][e_y]['cost'] == -1) or (smallest_cost < grid_cost[e_x][e_y]['cost'])):
        ...
        if smallest_cost <= grid_cost[x][y]['cost']:
            adj = [
                (n_x, n_y) for (n_x, n_y) in get_adjacent(x, y)
                if (
                        (grid_cost[n_x][n_y]['cost'] == -1) or
                        (grid_cost[n_x][n_y]['cost'] > (smallest_cost + 1))
                )
            ]
            if len(adj) > 0:
                if (smallest_cost + 1) not in known_costs:
                    prev = smallest_cost
                    while True:
                        if (known_costs[prev] == -1) or (known_costs[prev] > (smallest_cost + 1)):
                            ...
                            break
                if (smallest_cost + 1) not in frontier:
                    frontier[smallest_cost + 1] = []
                for (n_x, n_y) in adj:
                    grid_cost[n_x][n_y] = {'cost' : smallest_cost + 1, 'from' : (x, y)}
                    frontier[smallest_cost + 1].append((n_x, n_y))
        if len(frontier[smallest_cost]) == 0:
