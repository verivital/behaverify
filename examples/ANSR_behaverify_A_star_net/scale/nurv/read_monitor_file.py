...
class read_monitor(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        temp = collision_monitor_lib.user_interface(self.blackboard.drone_x, self.blackboard.drone_y, self.blackboard.drone_speed, self.blackboard.delta_x(), self.blackboard.delta_y())
        ...
        temp = loop_monitor_lib.user_interface(ACTION_TO_INT[self.blackboard.current_action], NEEDS_RESET)
        self.blackboard.monitor_var = ('safe' if temp == 1 else ('unknown' if temp == 0 else 'unsafe'))
