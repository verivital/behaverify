...
for category in "${categories[@]}"; do
	...
	python3 ./copilot/make_copilot_hs.py "${max_val}" "${num_obs}" "${category}/${tail_end}/obstaclesFilled_${tail_end}.txt" "${category}/${tail_end}/copilot/python/"
	echo "generating copilot monitors"
	python3 ../../../Docker_BehaVerify/Copilot/python_script/generate.py "${category}/${tail_end}/copilot/python/CollisionMonitor.hs" "${category}/${tail_end}/copilot/python/collision"
	python3 ../../../Docker_BehaVerify/Copilot/python_script/generate.py "${category}/${tail_end}/copilot/python/LoopMonitor.hs" "${category}/${tail_end}/copilot/python/loop"
	...
	python3 "${category}/${tail_end}/copilot/python/ANSR_runner.py" > "${category}/${tail_end}/copilot/python/trace.txt"
	echo "creating images"
	mkdir "${category}/${tail_end}/copilot/images"
	python3 "../parse_python_output.py" "${category}/${tail_end}/copilot/python/trace.txt" "${category}/${tail_end}/copilot/images/monitor" $((max_val + 1)) $((max_val + 1))
    done
done
