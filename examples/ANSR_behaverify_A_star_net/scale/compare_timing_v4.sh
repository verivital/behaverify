...
for category in "${categories[@]}"; do
	...
	for mon_type in "${mon_types[@]}"; do
		...
		hyperfine -N --warmup 200 --runs 10000 --export-json="${category}/${tail_end}/${mon_type}/python/TIMING_RESULT_v4.json" "${category}/${tail_end}/${mon_type}/python/timing 1000 ${max_val}"
	    else
	    ...
	    median=$(jq -r '.results[0].median' "${category}/${tail_end}/${mon_type}/python/TIMING_RESULT_v4.json")
	    echo "${max_val} ::> ${median}" >> "${category}/timing_v4_${mon_type}.txt"
	    sleep 1
	done
    done
done
