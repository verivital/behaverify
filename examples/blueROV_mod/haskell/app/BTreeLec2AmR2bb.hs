...
bTreeFunctionLec2AmR2bb :: [BTreeNode] -> TreeLocation -> TrueMemoryStatus -> [TrueMemoryStorage] -> PartialMemoryStatus -> [PartialMemoryStorage] -> BTreeBlackboard -> BTreeEnvironment -> FutureChanges -> BTreeNodeOutput
bTreeFunctionLec2AmR2bb _ nodeLocation _ _ _ _ oldBlackboard oldEnvironment futureChanges = (returnStatus, [], [], newBlackboard, newEnvironment, newFutureChanges)
  where
    ...
    boardEnvUpdate0 boardEnv
      ...
      where
        ...
        privateTempBoardEnv2 = (updateBoardLec2AmRSpeedWarning blackboard (((envLec2AmR environment) == "speed") || ((envLec2AmR environment) == "speed_pipe")), environment)
          where
            (blackboard, environment) = privateTempBoardEnv1
        privateTempBoardEnv3 = (updateBoardLec2AmRPipeWarning blackboard (((envLec2AmR environment) == "pipe") || ((envLec2AmR environment) == "speed_pipe")), environment)
          where
