...
"""
This node captures the output of the lec2 assurance monitor right
"""
...
class ToBlackboard(py_trees.behaviours.SetBlackboardVariable):
    ...
    Blackboard Variables:
        * lec2_am_r: the raw message from topic /lec2_am/right/p_value
        * lec2_am_r_speed_warning (:obj:`bool`)
        * lec2_am_r_pipe_warning (:obj:`bool`)
    Args:
    ...
    def __init__(self, 
                    name, 
                    topic_name="lec2_right_am",         
                    pipe_estimation_good_log_val=None,#<textx:btree.DefaultBBType instance at 0x7f9b3ea7dc70>,         
                    speed_good_log_val=None,#<textx:btree.DefaultBBType instance at 0x7f9b3ea7ddc0>        
                ):
                
        super(ToBlackboard, self).__init__(name=name,
                                           #topic_name=topic_name,
                                           #topic_type=LEC1OutputAssuredStamped,
                                           variable_name="lec2_am_r",
                                           variable_value=None
        ...
        #self.blackboard.lec2_am_r = LEC1OutputAssuredStamped()
        
        #self.blackboard.lec2_am_r_speed_warning = None
        #self.blackboard.lec2_am_r_pipe_warning = None
...
############<<USER UPDATE CODE BEGINS>>##############################
        log_m = np.log(self.blackboard.lec2_am_r.confs[0].values[-2])
        if log_m > self.speed_good_log_val * 2 :
            self.blackboard.lec2_am_r_speed_warning = True
            rospy.logwarn_throttle(1, "%s: lec2_am_r is high!" % self.name)
        elif log_m < self.speed_good_log_val:
            self.blackboard.lec2_am_r_speed_warning = False
        # else don't do anything in between - i.e. avoid the ping pong problems

        if log_m > self.pipe_estimation_good_log_val * 2 :
            self.blackboard.lec2_am_r_pipe_warning = True
            rospy.logwarn_throttle(1, "%s: lec2_am_r is high!" % self.name)
        elif log_m < self.pipe_estimation_good_log_val:
            self.blackboard.lec2_am_r_pipe_warning = False
        # else don't do anything in between - i.e. avoid the ping pong problems

        self.feedback_message = "lec2_am_r is high" if (self.blackboard.lec2_am_r_pipe_warning or self.blackboard.lec2_am_r_speed_warning) else "lec2_am_r is ok"
