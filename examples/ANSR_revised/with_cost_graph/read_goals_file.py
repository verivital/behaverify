...
class read_goals(py_trees.behaviour.Behaviour):
    def __init__(self, name, environment, convert_planner_to_bt):
        super(read_goals, self).__init__(name)
        self.name = name
        self.environment = environment
        self.convert_planner_to_bt = convert_planner_to_bt
    ...
    def update(self):
        if self.environment.read_goals_ready():
            ...
            self.blackboard.goals = self.blackboard.goals[0:new_goals_index] + [self.convert_planner_to_bt(self.blackboard.map_info, goal) for goal in new_goals]
        ...
        with open('/output/serene.log', 'a', encoding = 'utf-8') as serene_log:
            serene_log.write('read_goals: ' + str(return_status) + '\n')
