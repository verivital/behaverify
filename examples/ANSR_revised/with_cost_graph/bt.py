...
class BT(rclpy.node.Node):
    def __init__(self):
        ...
        safety_radius = 1 #if obstacle is within this distance using taxicab metric on x, y, then count as obstacle.
        safety_altitude = 1 #must be at least this distance above an obstacle to not be an obstacle
        flight_heights = [5, 25] #heights we can plan at
        update_radius = 12 #how much of the map we should update
        velocity = 10.0 #drone speed
        altitude_safety_margin = 1.25 #how far above the specified point we actually request to fly.

        # internal use only!
        def calculate_cost_function(point1, point2):
            return (1.5 if point1[0] != point2[0] and point1[1] != point2[1] else 1) * point2[2]

        # internal use only!
        def create_cost_list_at_point_function(map_info, obstacle_map, point):
            (x, y, z) = point
            return [
                ((x + x_mod, y + y_mod, z1), calculate_cost_function(point, (x + x_mod, y + y_mod, z1)))
                for x_mod in (-1, 0, 1)
                for y_mod in (-1, 0, 1)
                for z1 in flight_heights
                if (
                        (x_mod != 0 or y_mod !=0 or z != z1) and
                        (z == z1 or (x_mod == 0 and y_mod == 0)) and
                        0 <= x + x_mod < map_info['x_size'] and
                        0 <= y + y_mod < map_info['y_size'] and
                        not obstacle_at_point_function(map_info, obstacle_map, (x + x_mod, y + y_mod, z1))
                )
            ]

        # bt will use whatever obstacle map uses.
        def convert_planner_to_bt(map_info, point):
            return (
        ...
        def convert_odometry_to_bt(map_info, point):
            # NOT FINISHED
            return (
        ...
        def convert_bt_to_a_star(map_info, point):
            z = flight_heights[0]
        ...
        def convert_bt_to_waypoint(map_info, point, height_modifier):
            return (
        ...
        def near_goal_function(drone, goal):
            return (
                drone[2] >= goal[2] and
        ...
        def near_path_segment_end_function(drone, end_point):
            return near_goal_function(drone, end_point)

        def obstacle_at_point_function(map_info, obstacle_map, point):
        ...
        def reformat_obstacle_map_function(map_info, obstacle_map):
            return np.transpose(np.reshape(obstacle_map, (map_info['y_size'], map_info['x_size'])), (1, 0))

        def create_cost_graph_function(map_info, obstacle_map):
            return {
                (x, y, z) : create_cost_list_at_point_function(map_info, obstacle_map, (x, y, z))
                for x in range(map_info['x_size'])
                for y in range(map_info['y_size'])
                for z in flight_heights
            }

        def update_cost_graph_function(map_info, obstacle_map, cost_graph, point):
            for x in range(max(0, point[0] - update_radius), min(map_info['x_size'], point[0] + update_radius)):
                for y in range(max(0, point[1] - update_radius), min(map_info['y_size'], point[1] + update_radius)):
                    for z in flight_heights:
                        cost_graph[(x, y, z)] = create_cost_list_at_point_function(map_info, obstacle_map, (x, y, z))
            return cost_graph
