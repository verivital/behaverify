...
class action_read_obstacle_map(py_trees.behaviour.Behaviour):
    def __init__(self, name, environment, function_reformat_obstacle_map):
        super(action_read_obstacle_map, self).__init__(name)
        self.name = name
    ...
    def update(self):
        ...
        with open('/output/serene.log', 'a', encoding = 'utf-8') as serene_log:
            serene_log.write('action_read_obstacle_map: ' + str(return_status) + '\n')
