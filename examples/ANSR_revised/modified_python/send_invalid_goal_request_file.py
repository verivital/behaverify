...
class send_invalid_goal_request(py_trees.behaviour.Behaviour):
    def __init__(self, name, environment):
        super(send_invalid_goal_request, self).__init__(name)
        self.name = name
    ...
    def update(self):
        self.environment.send_invalid_goal_request_function__0(self)
        self.blackboard.goal_requested = False
        return_status = py_trees.common.Status.FAILURE
        with open('/output/serene.log', 'a', encoding = 'utf-8') as serene_log:
            serene_log.write('STATUS of send_invalid_goal_request: ' + str(return_status) + '\n')
