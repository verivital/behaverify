...
def create_tree():
    ...
    blackboard_reader.x = 0
    blackboard_reader.y = 0
    blackboard_reader.target_x = 0
    blackboard_reader.target_y = 0
    blackboard_reader.mission = False

    robot_control = py_trees.composites.Sequence('robot_control', memory = False)
    get_position = get_position_file.get_position('get_position')
    reset_completed_mission = py_trees.composites.Sequence('reset_completed_mission', memory = False)
    ...
    confirm_mission = py_trees.composites.Selector('confirm_mission', memory = False)
    ...
    move_robot = py_trees.composites.Selector('move_robot', memory = False)
    try_right = py_trees.composites.Sequence('try_right', memory = False)
