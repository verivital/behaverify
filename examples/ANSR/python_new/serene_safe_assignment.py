...
def prev_dest_x(new_value):
    ...
    if new_value >= 0 and new_value <= 10:
        ...
        raise ValueError('variable prev_dest_x expected value between 0 and 10 inclusive but received value ' + str(new_value))
...
def prev_dest_y(new_value):
    ...
    if new_value >= 0 and new_value <= 10:
        ...
        raise ValueError('variable prev_dest_y expected value between 0 and 10 inclusive but received value ' + str(new_value))
...
def dir(new_value):
    if type(new_value) is not int:
        raise TypeError('variable dir expected type int but received type ' + str(type(new_value)))
    if new_value in [-1, 1]:
        return new_value
    else:
        raise ValueError('variable dir expected value in [-1, 1] but received value ' + str(new_value))
