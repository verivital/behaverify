...
def create_decorator_condition(ignored_value):
    (status_start, status_end, active, active_end, children) = common_string(1)
    return_string = ("MODULE decorator_condition(child_status, status)" + os.linesep
                     + status_start
...
def create_decorator_inverter(ignored_value):    
    (status_start, status_end, active, active_end, children) = common_string(1)
    return_string = ("MODULE decorator_inverter(child_status)" + os.linesep
                     + status_start
                     + "\t\t\t\t(child_status in {invalid, error}) : error;" + os.linesep  #we found an error
...
def create_decorator_one_shot(ignored_value):
    ...
    return_string = ("MODULE decorator_one_shot(child_status,descendants, child_status, on_successful_completion_only)" + os.linesep
                      + status_start
                     + "\t\t\t\t(child_status in {invalid, error}) : error;" + os.linesep  #we found an error
...
def create_decorator_failure_is_running(ignored_value):
    (status_start, status_end, active, active_end, children) = common_string(1)
    return_string = ("MODULE decorator_failure_is_running(child_status)" + os.linesep
                     + status_start
                     + "\t\t\t\t(child_status in {invalid, error}) : error;" + os.linesep  #we found an error
...
def create_decorator_failure_is_success(ignored_value):
    (status_start, status_end, active, active_end, children) = common_string(1)
    return_string = ("MODULE decorator_failure_is_success(child_status)" + os.linesep
                     + status_start
                     + "\t\t\t\t(child_status in {invalid, error}) : error;" + os.linesep  #we found an error
                     + "\t\t\t\t!(child_status = failure) : child_status;" + os.linesep  #no matter what, we match the child
...
def create_decorator_running_is_failure(ignored_value):
    (status_start, status_end, active, active_end, children) = common_string(1)
    return_string = ("MODULE decorator_running_is_failure(child_status)" + os.linesep
                     + status_start
                     + "\t\t\t\t(child_status in {invalid, error}) : error;" + os.linesep  #we found an error
...
def create_decorator_running_is_success(ignored_value):
    (status_start, status_end, active, active_end, children) = common_string(1)
    return_string = ("MODULE decorator_running_is_success(child_status)" + os.linesep
                     + status_start
                     + "\t\t\t\t(child_status in {invalid, error}) : error;" + os.linesep  #we found an error
                     + "\t\t\t\t!(child_status = running) : child_status;" + os.linesep  #no matter what, we match the child
...
def create_decorator_success_is_failure(ignored_value):
    (status_start, status_end, active, active_end, children) = common_string(1)
    return_string = ("MODULE decorator_success_is_failure(child_status)" + os.linesep
                     + status_start
                     + "\t\t\t\t(child_status in {invalid, error}) : error;" + os.linesep  #we found an error
...
def create_decorator_success_is_running(ignored_value):
    (status_start, status_end, active, active_end, children) = common_string(1)
    return_string = ("MODULE decorator_success_is_running(child_status)" + os.linesep
                     + status_start
                     + "\t\t\t\t(child_status in {invalid, error}) : error;" + os.linesep  #we found an error
                     + "\t\t\t\t!(child_status = success) : child_status;" + os.linesep  #no matter what, we match the child
...
def create_node_parallel(max_children_plus_one):
    ...
    for child in range(max_children_plus_one):
        return_string += (active[child]
                          + "\t\t\t\t(" + str(child) + " > last_child) : FALSE;" + os.linesep
                          #if this is beyond the last child, it is not active (note that if there are no children, none of them will be active)
                          + "\t\t\t\t(skip_child[" + str(child) + "] = -2) : FALSE;" + os.linesep
