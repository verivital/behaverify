...
def create_blackboard():
    ...
    def forward():
        return (
            -1
            if blackboard_reader.have_flag else
            (
            1
        ))
...
def create_tree(environment):
    set_zone = set_zone_file.set_zone('set_zone', environment)
    flag_not_returned = flag_not_returned_file.flag_not_returned('flag_not_returned', environment)
    in_maze = in_maze_file.in_maze('in_maze')
    can_move_forward = can_move_forward_file.can_move_forward('can_move_forward', environment)
    go_forward = go_forward_file.go_forward('go_forward', environment)
    try_forward = py_trees.composites.Sequence(name = 'try_forward', memory = False, children = [can_move_forward, go_forward])
    can_move_side = can_move_side_file.can_move_side('can_move_side', environment)
    go_side = go_side_file.go_side('go_side', environment)
    try_side = py_trees.composites.Sequence(name = 'try_side', memory = False, children = [can_move_side, go_side])
    change_side = change_side_file.change_side('change_side', environment)
    ...
    go_forward_1 = go_forward_file.go_forward('go_forward_1', environment)
    ...
    can_move_side_1 = can_move_side_file.can_move_side('can_move_side_1', environment)
    go_side_1 = go_side_file.go_side('go_side_1', environment)
    try_side_1 = py_trees.composites.Sequence(name = 'try_side_1', memory = False, children = [can_move_side_1, go_side_1])
    change_side_1 = change_side_file.change_side('change_side_1', environment)
    ...
    never_need_side = never_need_side_file.never_need_side('never_need_side', environment)
    ...
    search_tile = search_tile_file.search_tile('search_tile', environment)
    can_move_side_2 = can_move_side_file.can_move_side('can_move_side_2', environment)
    go_side_2 = go_side_file.go_side('go_side_2', environment)
    try_side_2 = py_trees.composites.Sequence(name = 'try_side_2', memory = False, children = [can_move_side_2, go_side_2])
    change_side_2 = change_side_file.change_side('change_side_2', environment)
    ...
    go_forward_2 = go_forward_file.go_forward('go_forward_2', environment)
