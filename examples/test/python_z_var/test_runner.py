...
def print_blackboard(blackboard_reader):
    ...
    ret_string += indent(1) + 'z: ' + str(blackboard_reader.z) + os.linesep
    return ret_string
...
def run_tree():
    ...
    blackboard_reader = test.create_blackboard(serene_randomizer)
    environment = test_environment.test_environment(blackboard_reader)
    serene_randomizer.set_blackboard_and_environment(blackboard_reader, environment)
    test.initialize_blackboard(blackboard_reader)
    environment.initialize_environment()
    root = test.create_tree(environment)
