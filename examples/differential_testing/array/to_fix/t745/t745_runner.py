...
import py_trees
import t745
import t745_environment

blackboard_reader = t745.create_blackboard()
environment = t745_environment.t745_environment(blackboard_reader)
root = t745.create_tree(environment)
...
def print_environment():
    ...
    ret_string += indent(1) + 'envVAR3: ' + str(environment.envVAR3) + os.linesep
    ret_string += indent(1) + 'envFROZENVAR4: ' + str(environment.envFROZENVAR4) + os.linesep
    ret_string += indent(1) + 'envDEFINE6: ' + str([environment.envDEFINE6(x) for x in range(2)]) + os.linesep
    ret_string += indent(1) + 'envDEFINE7: ' + str([environment.envDEFINE7(x) for x in range(2)]) + os.linesep
    return ret_string
