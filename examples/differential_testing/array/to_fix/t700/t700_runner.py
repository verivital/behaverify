...
import py_trees
import t700
import t700_environment

blackboard_reader = t700.create_blackboard()
environment = t700_environment.t700_environment(blackboard_reader)
root = t700.create_tree(environment)
...
def print_blackboard():
    ...
    ret_string += indent(1) + 'blDEFINE5: ' + str([blackboard_reader.blDEFINE5(x) for x in range(2)]) + os.linesep
    ret_string += indent(1) + 'blDEFINE7: ' + str(blackboard_reader.blDEFINE7()) + os.linesep
    return ret_string
...
def print_environment():
    ...
    ret_string += indent(1) + 'envDEFINE8: ' + str([environment.envDEFINE8(x) for x in range(3)]) + os.linesep
    return ret_string


node_to_locals = {
    'a3' : [{'name' : 'localVAR2', 'is_func' : False, 'array_size' : None}],
    'a2' : [],
}
