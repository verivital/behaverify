...
rightUnknown _ nodeLocation _ _ _ _ blackboard environment futureChanges
  | (("unknown" == (sereneIndexTiles ((min 3 ((envXLoc blackboard environment) + 1)) + (4 * (envYLoc blackboard environment))) blackboard)) && ((envXLoc blackboard environment) /= 3)) = (Success, [], [], blackboard, environment, futureChanges)
