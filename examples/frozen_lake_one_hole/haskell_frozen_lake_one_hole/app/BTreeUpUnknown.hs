...
upUnknown _ nodeLocation _ _ _ _ blackboard environment futureChanges
  | (("unknown" == (sereneIndexTiles ((envXLoc blackboard environment) + (4 * (max 0 ((envYLoc blackboard environment) - 1)))) blackboard)) && ((envYLoc blackboard environment) /= 0)) = (Success, [], [], blackboard, environment, futureChanges)
