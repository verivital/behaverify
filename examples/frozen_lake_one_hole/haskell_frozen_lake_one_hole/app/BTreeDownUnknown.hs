...
downUnknown _ nodeLocation _ _ _ _ blackboard environment futureChanges
  | (("unknown" == (sereneIndexTiles ((envXLoc blackboard environment) + (4 * (min 3 ((envYLoc blackboard environment) + 1)))) blackboard)) && ((envYLoc blackboard environment) /= 3)) = (Success, [], [], blackboard, environment, futureChanges)
