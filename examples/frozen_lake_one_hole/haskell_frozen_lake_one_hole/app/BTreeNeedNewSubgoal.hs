...
needNewSubgoal _ nodeLocation _ _ _ _ blackboard environment futureChanges
  | (("hole" == (sereneIndexTiles ((boardXSubgoal blackboard) + (4 * (boardYSubgoal blackboard))) blackboard)) || ((envLoc environment) == (boardSubgoal blackboard))) = (Success, [], [], blackboard, environment, futureChanges)
