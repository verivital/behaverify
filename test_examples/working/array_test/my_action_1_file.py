...
class my_action_1(py_trees.behaviour.Behaviour):
    def __init__(self, name, environment):
        super(my_action_1, self).__init__(name)
        self.name = name
    ...
    def update(self):
        ...
        __temp_var__ = serene_safe_assignment.e([(min(2, max(self.blackboard.a[0], self.blackboard.b[0])), True), (min(2, max(self.blackboard.a[1], self.blackboard.b[1])), True), (min(2, max(self.blackboard.a[2], self.blackboard.b[2])), True)])
        for (index, val) in __temp_var__:
