...
def create_decorator_condition(number_of_nodes):
    ...
    return("MODULE decorator_condition(relevant_child, relevant_status, status)" + os.linesep
                      + status_define
...
def create_decorator_inverter(number_of_nodes):
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE decorator_inverter(relevant_child, relevant_status)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
...
def create_decorator_one_shot(number_of_nodes):
    ...
    return("MODULE decorator_one_shot(relevant_child, relevant_status,descendants, relevant_status, on_successful_completion_only)" + os.linesep
                      + status_define
...
def create_decorator_failure_is_running(number_of_nodes):
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE decorator_failure_is_running(relevant_child, relevant_status)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
...
def create_decorator_failure_is_success(number_of_nodes):
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE decorator_failure_is_success(relevant_child, relevant_status)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t!(relevant_status = failure) : relevant_status;" + os.linesep  #no matter what, we match the child
...
def create_decorator_running_is_failure(number_of_nodes):
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE decorator_running_is_failure(relevant_child, relevant_status)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
...
def create_decorator_running_is_success(number_of_nodes):
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE decorator_running_is_success(relevant_child, relevant_status)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t!(relevant_status = running) : relevant_status;" + os.linesep  #no matter what, we match the child
...
def create_decorator_success_is_failure(number_of_nodes):
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE decorator_success_is_failure(relevant_child, relevant_status)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
...
def create_decorator_success_is_running(number_of_nodes):
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE decorator_success_is_running(relevant_child, relevant_status)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t!(relevant_status = success) : relevant_status;" + os.linesep  #no matter what, we match the child
...
def create_blackboard(int_to_variable, variable_to_int, variable_access, nodes, no_seperate_modules, min_val = 0, max_val = 1):
    ...
    if int_to_variable and not no_seperate_modules:
        ...
        for i in range(len(int_to_variable)):
            ...
            if variable_to_int[variable] in variable_access:
                for node in variable_access[variable_to_int[variable]]:
                    assign_string += "\t\t\t\t(active_node = node_names." + nodes[node][2] + ") & (statuses[active_node] = success) : " + poss_values + ";" + os.linesep
...
def create_node_check_blackboard_variable_exists(number_of_nodes):
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_check_blackboard_variable_exists(active_node, id, blackboard, variable)" + os.linesep
                      + status_define
...
def create_node_check_blackboard_variable_value(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_check_blackboard_variable_value(active_node, id, blackboard, variable, check)" + os.linesep
                      + status_define
...
def create_node_check_blackboard_variable_values(number_of_nodes, variables, checks, operators):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_check_blackboard_variable_values(active_node, id, variables_exist, checks)" + os.linesep
                      + status_define
...
def create_node_set_blackboard_variable(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_set_blackboard_variable(active_node, id, blackboard, variable, nodes_with_access, value_creator, overwrite)" + os.linesep
                      + status_define
...
def create_node_set_blackboard_variables(number_of_nodes):
    ...
    return("MODULE node_set_blackboard_variables(active_node, id, status_module)" + os.linesep
                      + "\tDEFINE" + os.linesep
...
def create_node_unset_blackboard_variable(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_unset_blackboard_variable(active_node, id, variable, nodes_with_access)" + os.linesep
                      + status_define
...
def create_node_wait_for_blackboard_variable(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_wait_for_blackboard_variable(active_node, id, variable, blackboard)" + os.linesep
                      + status_define
...
def create_node_wait_for_blackboard_variable_value(number_of_nodes, variable, value):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_wait_for_blackboard_variable_value(active_node, id, blackboard, variable, check)" + os.linesep
                      + status_define
...
def create_node_count(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_count(active_node, id, fail_until, running_until, success_until, reset)" + os.linesep
                      + status_define
...
def create_node_failure(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_failure(active_node, id)" + os.linesep
                      + status_define
...
def create_node_periodic(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_periodic(active_node, id, period)" + os.linesep
                      + status_define
...
def create_node_running(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_running(active_node, id)" + os.linesep
                      + status_define
...
def create_node_status_sequence(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_status_sequence(active_node, id, sequence, sequence_length, eventually)" + os.linesep
                      + status_define
...
def create_node_success(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_success(active_node, id)" + os.linesep
                      + status_define
                      + "\t\t\t\t(active_node = id) : success;" + os.linesep  #this always returns success
                      + status_end)
...
def create_node_success_every_n(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_success_every_n(active_node, id, n)" + os.linesep
                      + status_define
...
def create_node_tick_counter(number_of_nodes): 
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_tick_counter(active_node, id, duration, completion_status)" + os.linesep
                      + status_define
...
def create_node_non_blocking(number_of_nodes):
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_non_blocking(active_node, id, random_status)" + os.linesep
           + status_define
           + "\t\t\t\t(active_node = id) & (random_status = success) : random_status;" + os.linesep #return any status
           + "\t\t\t\t(active_node = id) : failure;" + os.linesep #return any status
           + status_end
    )
...
def create_node_timer(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_timer(active_node, id, random_status)" + os.linesep
                      + status_define
...
def create_node_default(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_default(active_node, id, random_status)" + os.linesep
                      + status_define
...
def create_node_selector(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_selector(relevant_child, relevant_status, last_child)" + os.linesep 
                      + status_define
...
def create_node_sequence(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_sequence(relevant_child, relevant_status, last_child)" + os.linesep 
                      + status_define
...
def create_node_parallel(number_of_nodes):
    ...
    (status_define, status_end) = common_string(number_of_nodes)
    return("MODULE node_parallel(relevant_child, relevant_status, last_child, reset, synchronized, parallel_policy_all)" + os.linesep
                      + status_define
                      + "\t\t\t\t(last_child = -2) : success;" + os.linesep#no children, return default
                      + "\t\t\t\t(relevant_child <= 0) : invalid;" + os.linesep#current node is not our descendant, we're invalid
