...
def make_new_stage(statement, node_name, variables, local_variables, use_stages, use_next, not_next):
    ...
    '''
    # first we go through all the not default case-results, and pair them up.
    # then we add the base case.
...
def format_function_status(function_name, code, node_name, variables, local_variables, use_stages, use_next, not_next):
    return (
        '((!(active_node = node_names.' + code.function_call.node_name + ')) -> (current_status = ' + function_name + '))'
        )
...
def create_check(current_node, node_name, parent_name, variables, local_variables, delayed_statements):
    ...
    return create_node_template(node_name, parent_name, 'leaf', current_node.node_type,
                                ...
                                internal_status_module_code = (
                                    [
                                        (format_code(current_node.condition, node_name, variables, local_variables, False, False, None), 'success'),
                                        ('TRUE', 'failure')
                                    ]
                                ))
...
def create_action(current_node, node_name, parent_name, variables, local_variables, delayed_statements):
            ...
            if cur_statement.variable_statement is not None:
                ...
                if init_statement:
                    ...
                    for variable_statement in statement.variable_statements:
                        variable_key = variable_reference(variable_statement.variable.name, variable_statement.is_local, node_name, False)
                        if variable_key not in variables:
    ...
    node = create_node_template(node_name, parent_name, 'leaf', 'action',
                                ...
                                (
                                    [
                                        ((format_code(case_result.condition, node_name, variables, local_variables, False, False, None), case_result.status))
                                        for case_result in current_node.return_statement.case_results
                                    ]
                                    +
                                    [
                                        ('TRUE', current_node.return_statement.default_result.status)
                                    ]
                                ))
...
def handle_specifications(specifications, variables, local_variables):
    return [
        ...
        for specification in specifications if specification.spec_type != 'INVARSPEC'
        ]
