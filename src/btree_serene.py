#python3 btree_serene.py create_root_FILE create_root_METHOD --root_args create_root_ARGS  --min_val min_val_INT --max_val max_val_INT --modules additional_modules_FILE --specs specs_FILE


#TODO: neither parallel works correctly right now. implement both.
...
def walk_tree(t,parent_id, assigned_id, sub_parallel = -1, sub_child = -1):
    global node_count, children, nodes, additional_arguments, needed_nodes, variable_to_int, variable_access, blackboard_needed, last_child, parallel_resume, parallel_synch_list, parallel_unsych_list;
    this_id=assigned_id
    try:
        ...
        if t.serene_info_variable == "BlueROV_Blackboard_Node" or t.serene_info_variable == "BlueROV_Task_Node":
            ...
            if blackboard_match:
                ...
                if var_num in variable_access:
                    ...
                    variable_access[var_num]=[this_id]
            decl_string = ("\t\t" + node_name + "_SET_status : " + node_name + "_SET_status_module(active_node, " + str(this_id) + ", blackboard, node_names, variable_names);" + os.linesep)
        ...
        additional_arguments[this_id]=[{'arg' : 'blackboard'},
                                       {'arg' : str(var_num)},
                                       {
                                           ...
                                           'decl' : ('\t\t' + node_name + '_CHECK_' + variable_name
                                                     + ' : '+ node_name + '_CHECK_' + variable_name + '_module(active_node, ' + str(this_id) + ', blackboard, variable_names);' + os.linesep)
                                       }]
        return
    elif isinstance(t, py_trees.behaviours.CheckBlackboardVariableValues):
        #(statuses, active_node, id, parent, variables_exist, checks)" + os.linesep
...
def main():
    ...
    arg_parser.add_argument('--max_val', default=100, type=int)
    ...
    for variable in variable_to_int:
        print('--'+variable+' : '+str(variable_to_int[variable]))
        if variable_to_int[variable] in variable_access:
            for access_node in variable_access[variable_to_int[variable]]:
                print('----' + nodes[access_node][2])
    if modules:
