...
def resolve_statements(statements, nodes, variables):
    ...
    def handle_return_statement(statement, nodes, node_name, variables):
        ...
        node['internal_status_module_code'] = (
            ...
            (
                ...
                + ('').join([(tab_indent(4)
                            + format_code(case_result.condition, node_name, variables, False, False, None)
                              + ' : '
                              + case_result.status
                              + ';' + os.linesep)
                             for case_result in statement.case_results])
    ...
    def handle_condition(condition, nodes, node_name, variables):
        ...
        node['internal_status_module_code'] = (
            ...
            + tab_indent(4) + 'reset : invalid;' + os.linesep
            + tab_indent(4) + '!(active) : status;' + os.linesep
            + tab_indent(4) + format_code(condition, node_name, variables, False, False, None) + ' : success;' + os.linesep
