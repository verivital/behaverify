...
sequence_point = {} # a map from sequence_node_id to which resume_status is looked at
sequence_map = {} #indexed by sequence_node_id. maps a dictionary. for every descendant leaf node, points to itself or the highest selector between the two
...
def leaf_node_exit(node_id):
    global nodes, sequence_map, sequence_point, advanced_resume;

    cur_id = node_id
    most_recent_selector = node_id#not actually a selector, but that doesn't matter
    ...
    while not done:
        ...
        elif nodes[cur_id][0] == "node_parallel":
            done = True
        elif nodes[cur_id][0] == "node_selector":
            if not encountered_sequences:
                node_id=cur_id#collapse everything beneath a selector into the selector, if we can. (can't if we on a sequence path)
            most_recent_selector = cur_id
        elif nodes[cur_id][0] == "node_sequence":
            encountered_sequences.append(cur_id)
            if cur_id not in sequence_map:
                sequence_map[cur_id] = {}
            if children[cur_id][0] == previous_id:
                pass
            else:
                first_child = False#we just encountered proof that this is not a first child case. it is important to track.
            if not first_child:
                sequence_map[cur_id][node_id] = most_recent_selector
    if encountered_sequences and not first_child:
        if node_id not in advanced_resume[previous_id]:
            advanced_resume[previous_id].append(node_id)
    for seq in encountered_sequences:
        sequence_point[seq] = previous_id
...
def walk_tree(t, parent_id, assigned_id,  parallel_unsynch = False):
    ...
    node_count=node_count+len(t.children)#skip ahead to this point. we'll fill in the parts we miss as we iterate over the children.
    #----------------------------------------------------------------------------------------------------------------------
...
def main():
    ...
    """
    global node_count, children, nodes, additional_arguments, needed_nodes, variable_to_int, variable_access, blackboard_needed, last_child, advanced_resume, sequence_point, sequence_map, parallel_synch_list, parallel_unsych_list, variable_check, external_status_req;
    ...
    for index in advanced_resume:
        if len(advanced_resume[index]) == 0:
            ...
            #or the default root node. i guess
    ...
    for node_id in range(0, node_count):
        ...
        if node_id not in children:
        ...
        elif len(children[node_id]) == 0:
            #should never run, but w.e
            #just in case someone adds a selector with no child or something.
            post_pre_nuxmv_string += ("\t\t\t\t!(active_node = " + str(node_id) + ") : " + str(node_id) + ";" + os.linesep)#point to this node unless we're at this node.
            #else, point to parent
            ...
            for descendant in sequence_map[node_id]:
                post_pre_nuxmv_string += ("\t\t\t\t(resume_status_" + str(sequence_point[node_id]) + " = " + str(descendant) + ") : next_node_" + str(sequence_map[node_id][descendant]) + ";" + os.linesep #if the resume points to one of our 'leafs', then jump to wherever it's pointing.
                )
            for child in children[node_id]:
                post_pre_nuxmv_string += ("\t\t\t\t(previous_node <" + str(child) + ") : next_node_" + str(child) + ";" + os.linesep)#otherwise, go through each child in order
