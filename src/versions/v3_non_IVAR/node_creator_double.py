...
def create_decorator_condition(number_of_nodes):
    ...
    return_string += ("MODULE decorator_condition(relevant_child, statuses, status)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = status) : success;" + os.linesep  #it matches the status
                      + "\t\t\t\t!(statuses[max(relevant_child, 0)] = status) : running;" + os.linesep  #it doesn't match the status
                      + status_end)
...
def create_decorator_inverter(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE decorator_inverter(relevant_child, statuses)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t((statuses[max(relevant_child, 0)] = invalid) | (statuses[max(relevant_child, 0)] = running)) : statuses[max(relevant_child, 0)];" + os.linesep  #child is either running or invalid. either way, that's what we are
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = failure) : success;" + os.linesep  #failure becomes success
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = success) : failure;" + os.linesep  #success becomes failure
                      + status_end)
    return return_string
def create_decorator_one_shot(number_of_nodes):
    ...
    return_string += ("MODULE decorator_one_shot(relevant_child, statuses,descendants, statuses, on_successful_completion_only)" + os.linesep
                      + status_define
                      ...
                      + "\t\t\tcase" + os.linesep
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = success) : TRUE;" + os.linesep#set to true if child succeeded
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = failure) & !(on_successful_completion_only) : TRUE;" + os.linesep#also set to true if child failed, but that's ok
                      + "\t\t\t\tTRUE : child_completed;" + os.linesep#otherwise, hold
                      + "\t\t\tesac;" + os.linesep
                      + "\t\tnext(child_final) :=" + os.linesep
                      + "\t\t\tcase" + os.linesep
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = success) : success;" + os.linesep#if we returned success, explicitly set it to success
                      ...
                      + "\t\t\t\t!(child_completed) : statuses[max(relevant_child, 0)];" +os.linesep
                      + status_end
                      + "\t\t\t\t(child_completed) : parent;" + os.linesep #child already finished. time to leave
                      + "\t\t\t\t!(child_completed) & (statuses[max(relevant_child, 0)] = invalid) : child;" + os.linesep #if the child has not yet been ran, go run the child.
                      + "\t\t\t\t!(child_completed) & !(statuses[max(relevant_child, 0)] = invalid) : parent;" + os.linesep # child already ran. time to return to parent
...
def create_decorator_failure_is_running(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE decorator_failure_is_running(relevant_child, statuses)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t!(statuses[max(relevant_child, 0)] = failure) : statuses[max(relevant_child, 0)];" + os.linesep  #no matter what, we match the child
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = failure) : running;" + os.linesep  #unless it failed. then we pretend it's still running
                      + status_end)
    return return_string
def create_decorator_failure_is_success(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE decorator_failure_is_success(relevant_child, statuses)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t!(statuses[max(relevant_child, 0)] = failure) : statuses[max(relevant_child, 0)];" + os.linesep  #no matter what, we match the child
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = failure) : success;" + os.linesep  #unless it failed. then we pretend it succeeded
                      + status_end)
    return return_string
def create_decorator_running_is_failure(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE decorator_running_is_failure(relevant_child, statuses)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t!(statuses[max(relevant_child, 0)] = running) : statuses[max(relevant_child, 0)];" + os.linesep  #no matter what, we match the child
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = running) : failure;" + os.linesep  #unless it's running. that's a fail
                      + status_end)
    return return_string
def create_decorator_running_is_success(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE decorator_running_is_success(relevant_child, statuses)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t!(statuses[max(relevant_child, 0)] = running) : statuses[max(relevant_child, 0)];" + os.linesep  #no matter what, we match the child
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = running) : success;" + os.linesep  #unless it running. that's a success
                      + status_end)
    return return_string
def create_decorator_success_is_failure(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE decorator_success_is_failure(relevant_child, statuses)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t!(statuses[max(relevant_child, 0)] = success) : statuses[max(relevant_child, 0)];" + os.linesep  #no matter what, we match the child
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = success) : failure;" + os.linesep  #we won't let you succeed
                      + status_end)
    return return_string
def create_decorator_success_is_running(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE decorator_success_is_running(relevant_child, statuses)" + os.linesep
                      + status_define
                      + "\t\t\t\t(relevant_child < 0) : invalid;" + os.linesep #must be considering the child.
                      + "\t\t\t\t!(statuses[max(relevant_child, 0)] = success) : statuses[max(relevant_child, 0)];" + os.linesep  #no matter what, we match the child
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = success) : running;" + os.linesep  #we won't let you succeed
                      + status_end)
    return return_string
...
def create_node_selector(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE node_selector(relevant_child, statuses, last_child)" + os.linesep 
                      + status_define
                      ...
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = running) : running;" + os.linesep#a child returned running. therefore, we are running
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = success) : success;" + os.linesep#a child returned success. therefore, we are success
                      + "\t\t\t\t(last_child = relevant_child) & (statuses[max(relevant_child, 0)] = failure) : failure;" + os.linesep#if it's the last child, we have failure
                      + status_end)
    return return_string
...
def create_node_sequence(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE node_sequence(relevant_child, statuses, last_child)" + os.linesep 
                      + status_define
                      + "\t\t\t\t(last_child = -2) : success;" + os.linesep#no children, return default
                      + "\t\t\t\t(relevant_child <= 0) : invalid;" + os.linesep#current node is not our descendant, we're invalid
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = running) : running;" + os.linesep#a child returned running. therefore, we are running
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = failure) : failure;" + os.linesep#a child returned failure, therefore, we are failure
                      + "\t\t\t\t(last_child = relevant_child) & (statuses[max(relevant_child, 0)] = success) : success;" + os.linesep#if it's the last child, we have success
                      + status_end)
    return return_string
...
def create_node_parallel(number_of_nodes):
    return_string=''
    (status_define, status_end) = common_string(number_of_nodes)
    return_string += ("MODULE node_parallel(relevant_child, statuses, last_child, reset, synchronized, parallel_policy_all)" + os.linesep
                      + status_define
                      + "\t\t\t\t(last_child = -2) : success;" + os.linesep#no children, return default
                      + "\t\t\t\t(relevant_child <= 0) : invalid;" + os.linesep#current node is not our descendant, we're invalid
                      + "\t\t\t\t!(relevant_child = last_child) : invalid;" + os.linesep#are we on the last child
                      #we are now on the last child
                      + "\t\t\t\t(statuses[max(relevant_child, 0)] = invalid) : invalid;" + os.linesep#if child is invalid, then we are invalid (this is handled by default case in sel, seq)
                      + "\t\t\t\t((cur_status = failure) | (statuses[max(relevant_child, 0)] = failure)) : failure;" + os.linesep#if we had any failures, return failure
                      #at this point, on last child, all children success or running
                      + "\t\t\t\t(cur_status = running) & (statuses[max(relevant_child, 0)] = running) : running;" + os.linesep #we were going to return running, and the last status was running, return running
                      ...
                      + "\t\t\t\t(cur_status = success) & (statuses[max(relevant_child, 0)] = success) : success;" + os.linesep#all success
                      ...
                      + "\t\t\t\t((statuses[max(relevant_child, 0)] = failure) | (cur_status = failure)) : failure;" + os.linesep#if a failure is present, stay on failure.
                      + "\t\t\t\t(parallel_policy_all) & (statuses[max(relevant_child, 0)] = success) : cur_status;" + os.linesep#if a success is present, continue with w/e we have
                      + "\t\t\t\t(parallel_policy_all) & (statuses[max(relevant_child, 0)] = running) : running;" + os.linesep#if a runnign is present, set to running
                      + "\t\t\t\t!(parallel_policy_all) & (statuses[max(relevant_child, 0)] = success) : success;" + os.linesep#if a success is present, set to success
                      + "\t\t\t\t!(parallel_policy_all) & (statuses[max(relevant_child, 0)] = running) : cur_status;" + os.linesep#if a runnign is present, continue with w/e we have
