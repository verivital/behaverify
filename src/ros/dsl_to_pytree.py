...
def build_read_action_node(node):
    return (STANDARD_IMPORTS
            + custom_imports(node)
            + class_definition_read_action(node.name)
            + init_method_read_action(node)
            + update_method_read_action(node)
            )
...
def main():
    ...
    for read_action in model.read_action_nodes:
        with open(args.location + read_action.name + '_file.py', 'w') as f:
            f.write(build_read_action_node(read_action))
